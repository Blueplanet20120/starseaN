...
struct starsead_runtime {
    struct starsead_runtime_reactor_backend backend;
    void *backend_context;
    bool dispatching;
    const struct starsead_config *config;
    struct starsead_state_document *state;
    struct starsead_control_live_context live;
    const struct starsead_runtime_effect_backend *effects;
    struct starsead_lifecycle lifecycle;
    struct starsead_runtime_delta pending_delta;
    bool supervising;
    bool starting_event_published;
};

void starsead_poll_builder_init(struct starsead_poll_builder *builder) {
    if (builder != NULL) memset(builder, 0, sizeof(*builder));
}

int starsead_poll_builder_add(
    struct starsead_poll_builder *builder, const struct starsead_poll_source *source) {
    if (builder == NULL || source == NULL || source->fd < 0 || source->events == 0 ||
        source->kind < STARSEAD_POLL_SIGNAL ||
        source->kind >= STARSEAD_POLL_SOURCE_KIND_COUNT || source->generation == 0U ||
        builder->count >= STARSEAD_MAX_POLL_SOURCES) return STARSEAD_CONFIG_INVALID;
    builder->sources[builder->count++] = *source;
    return 0;
}

bool starsead_process_poll_source_matches(
    const struct starsead_child_process *process, const struct starsead_poll_source *source) {
    if (process == NULL || source == NULL || process->pid <= 0 ||
        source->generation != (uint64_t)process->pid) return false;
    switch (source->kind) {
        case STARSEAD_POLL_PROCESS_EXEC_ERROR:
            return process->owns_setup_status_fd && source->fd == process->setup_status_fd;
        case STARSEAD_POLL_PROCESS_STDOUT:
            return process->owns_stdout_fd && source->fd == process->stdout_fd;
        case STARSEAD_POLL_PROCESS_STDERR:
            return process->owns_stderr_fd && source->fd == process->stderr_fd;
        case STARSEAD_POLL_PROCESS_PIDFD:
...
void starsead_deadline_min(
    struct starsead_deadline *target, const struct starsead_deadline *candidate) {
...
static unsigned runtime_dispatch_priority(enum starsead_poll_source_kind);

#if defined(STARSEAD_TESTING)
unsigned starsead_test_runtime_dispatch_priority(
    enum starsead_poll_source_kind kind) {
    return runtime_dispatch_priority(kind);
}

int starsead_test_periodic_deadline(
    int64_t now, int64_t caller_deadline, uint32_t interval,
    int64_t *iteration_deadline) {
...
void starsead_runtime_delta_init(struct starsead_runtime_delta *delta) {
    if (delta == NULL) return;
    memset(delta, 0, sizeof(*delta));
    delta->stop_reason = STARSEAD_LIFECYCLE_REASON_NONE;
}

static bool runtime_backend_valid(const struct starsead_runtime_reactor_backend *backend) {
    return backend != NULL && backend->monotonic_milliseconds != NULL &&
        backend->prepare != NULL && backend->wait != NULL && backend->dispatch != NULL &&
        backend->expire != NULL;
}

int starsead_runtime_create(struct starsead_runtime **output,
    const struct starsead_runtime_reactor_backend *backend, void *backend_context) {
    if (output == NULL) return STARSEAD_CONFIG_INVALID;
    *output = NULL;
    if (!runtime_backend_valid(backend)) return STARSEAD_CONFIG_INVALID;
    struct starsead_runtime *runtime = calloc(1U, sizeof(*runtime));
    if (runtime == NULL) return STARSEAD_CONFIG_NO_MEMORY;
...
static int64_t deadline_timeout(
    int64_t now, const struct starsead_deadline *deadline) {
...
static unsigned runtime_dispatch_priority(enum starsead_poll_source_kind kind) {
    switch (kind) {
        case STARSEAD_POLL_SIGNAL: return 0U;
        case STARSEAD_POLL_PROCESS_EXEC_ERROR:
        case STARSEAD_POLL_PROCESS_STDOUT:
        case STARSEAD_POLL_PROCESS_STDERR: return 1U;
        case STARSEAD_POLL_PROCESS_PIDFD: return 2U;
        case STARSEAD_POLL_CONTROL_LISTENER:
        case STARSEAD_POLL_CONTROL_CLIENT: return 3U;
...
int starsead_runtime_pump_once(struct starsead_runtime *runtime,
    const struct starsead_deadline *caller_deadline, struct starsead_runtime_delta *delta) {
    if (runtime == NULL || delta == NULL || runtime->dispatching ||
        (caller_deadline != NULL && caller_deadline->armed &&
            caller_deadline->monotonic_milliseconds < 0)) return STARSEAD_CONFIG_INVALID;

    starsead_runtime_delta_init(delta);
    runtime->dispatching = true;
    int result = STARSEAD_CONFIG_IO;
    int64_t now = 0;
    struct starsead_poll_builder builder;
    struct starsead_deadline module_deadline = {0};
    short ready[STARSEAD_MAX_POLL_SOURCES];
    starsead_poll_builder_init(&builder);
    ...
    if (runtime->backend.prepare(
        ...
        builder.count > STARSEAD_MAX_POLL_SOURCES ||
        (module_deadline.armed && module_deadline.monotonic_milliseconds < 0)) goto done;

    struct starsead_deadline wait_deadline = module_deadline;
    starsead_deadline_min(&wait_deadline, caller_deadline);
    int wait_result = runtime->backend.wait(runtime->backend_context, &builder,
        deadline_timeout(now, &wait_deadline), ready);
    if (wait_result == STARSEAD_REACTOR_WAIT_ERROR) goto done;
    if (wait_result != STARSEAD_REACTOR_WAIT_INTERRUPTED && wait_result < 0) goto done;

    if (wait_result != STARSEAD_REACTOR_WAIT_INTERRUPTED) {
...
int starsead_runtime_pump_until(struct starsead_runtime *runtime,
    const struct starsead_deadline *deadline, starsead_runtime_predicate predicate,
    void *predicate_context, struct starsead_runtime_delta *delta) {
    if (runtime == NULL || deadline == NULL || !deadline->armed ||
        deadline->monotonic_milliseconds < 0 || predicate == NULL || delta == NULL ||
        runtime->dispatching) return STARSEAD_CONFIG_INVALID;
    starsead_runtime_delta_init(delta);
    for (;;) {
        ...
        if (now >= deadline->monotonic_milliseconds) return STARSEAD_CONFIG_IO;
        struct starsead_runtime_delta iteration;
        int result = starsead_runtime_pump_once(runtime, deadline, &iteration);
        if (result != 0) return result;
        runtime_delta_merge(delta, &iteration);
        if ((iteration.flags & (STARSEAD_DELTA_STOP_REQUESTED |
                STARSEAD_DELTA_CHILD_EXITED | STARSEAD_DELTA_FATAL)) != 0U) return 0;
    }
}

static bool runtime_effects_valid(const struct starsead_runtime_effect_backend *effects,
    const struct starsead_config *config) {
    ...
    if (config->helper.type != STARSEAD_HELPER_NONE &&
        (effects->start_helper == NULL || effects->wait_helper == NULL ||
         effects->stop_helper == NULL)) return false;
    if (config->helper.type == STARSEAD_HELPER_BPF2SOCKS &&
        effects->ensure_platform_capability == NULL) return false;
    return true;
}

static int runtime_save(struct starsead_runtime *runtime) {
    return runtime->effects->save_state(runtime->effects->context, runtime->state) == 0
        ? 0 : STARSEAD_CONFIG_IO;
}

static int runtime_set_phase(struct starsead_runtime *runtime, enum starsead_phase phase) {
    if (starsead_state_set_phase(runtime->state, phase) != STARSEAD_STATE_OK) {
...
static int runtime_publish_event(
    struct starsead_runtime *runtime, enum starsead_control_event_type type) {
    struct starsead_control_snapshot snapshot;
    memset(&snapshot, 0, sizeof(snapshot));
    if (starsead_control_snapshot_from_state(
            runtime->state, &runtime->live, &snapshot) != 0) return STARSEAD_CONFIG_INVALID;
    const struct starsead_control_error *details = snapshot.has_error ? &snapshot.error : NULL;
    ...
    return result == 0 ? 0 : STARSEAD_CONFIG_IO;
}

static int runtime_save_child(
    struct starsead_runtime *runtime, const struct starsead_child_identity *identity) {
    char error[128U] = {0};
    if (starsead_state_set_child(
            runtime->state, identity, error, sizeof(error)) != STARSEAD_STATE_OK) {
        return STARSEAD_CONFIG_INVALID;
    }
    return runtime_save(runtime);
}
...
static int runtime_lifecycle_acquire(void *opaque) {
    struct starsead_runtime *runtime = opaque;
    starsead_state_clear_failure(runtime->state);
    return starsead_state_set_matcher(
        runtime->state, runtime->config->matcher.enabled, false);
}

static int runtime_lifecycle_start_core(void *opaque) {
    struct starsead_runtime *runtime = opaque;
    if (runtime_set_phase(runtime, STARSEAD_PHASE_STARTING) != 0) return STARSEAD_CONFIG_IO;
    if (!runtime->starting_event_published) {
        if (runtime_publish_event(runtime, STARSEAD_CONTROL_EVENT_STARTING) != 0) {
            return STARSEAD_CONFIG_IO;
        }
    ...
    if (runtime->effects->start_core(runtime->effects->context, &identity) != 0) {
        return STARSEAD_LIFECYCLE_START_FAILED;
    }
...
static int runtime_lifecycle_start_matcher(void *opaque) {
    ...
    if (runtime->effects->start_matcher(runtime->effects->context) != 0 ||
        starsead_state_set_matcher(runtime->state, true, true) != STARSEAD_STATE_OK) {
        return STARSEAD_LIFECYCLE_START_FAILED;
    }
    return runtime_save(runtime);
}
...
static int runtime_commit_rules(struct starsead_runtime *runtime, bool active,
    uint64_t generation, uint32_t categories, bool publish) {
    if (starsead_state_set_rules(
            runtime->state, active, generation, categories) != STARSEAD_STATE_OK ||
        runtime_save(runtime) != 0) return STARSEAD_CONFIG_IO;
    if (publish && active && !starsead_mode_core_managed(runtime->config->mode)) {
        return runtime_publish_event(runtime, STARSEAD_CONTROL_EVENT_RULES_CHANGED);
    }
    return 0;
}
...
static int runtime_lifecycle_apply_rules(void *opaque) {
    struct starsead_runtime *runtime = opaque;
    if (runtime_set_phase(runtime, STARSEAD_PHASE_APPLYING_RULES) != 0) {
        return STARSEAD_CONFIG_IO;
    }
    bool active = false;
...
static int runtime_lifecycle_verify(void *opaque) {
    ...
    if (runtime_set_phase(runtime, STARSEAD_PHASE_RUNNING) != 0) return STARSEAD_CONFIG_IO;
    return runtime_publish_event(runtime, STARSEAD_CONTROL_EVENT_RUNNING);
}
...
static int runtime_lifecycle_remove_rules(void *opaque) {
    struct starsead_runtime *runtime = opaque;
    if (runtime->effects->remove_rules(runtime->effects->context) != 0) return STARSEAD_CONFIG_IO;
...
static int runtime_lifecycle_stop_matcher(void *opaque) {
    ...
    if (runtime->effects->stop_matcher(runtime->effects->context) != 0 ||
        starsead_state_set_matcher(
            runtime->state, runtime->config->matcher.enabled, false) != STARSEAD_STATE_OK) {
        return STARSEAD_CONFIG_IO;
    }
    return runtime_save(runtime);
}

static int runtime_clear_child(struct starsead_runtime *runtime, enum starsead_child_role role) {
    if (starsead_state_clear_child(runtime->state, role) != STARSEAD_STATE_OK) {
        return STARSEAD_CONFIG_INVALID;
    }
    return runtime_save(runtime);
}
...
static int runtime_lifecycle_stop_helper(void *opaque) {
    struct starsead_runtime *runtime = opaque;
    if (runtime->effects->stop_helper(runtime->effects->context) != 0) return STARSEAD_CONFIG_IO;
    return runtime_clear_child(runtime, STARSEAD_CHILD_HELPER);
}

static int runtime_lifecycle_stop_core(void *opaque) {
    struct starsead_runtime *runtime = opaque;
    if (runtime->effects->stop_core(runtime->effects->context) != 0) return STARSEAD_CONFIG_IO;
    if (runtime_clear_child(runtime, STARSEAD_CHILD_CORE) != 0) return STARSEAD_CONFIG_IO;
    return 0;
}
...
static const struct starsead_lifecycle_backend runtime_lifecycle_backend = {
...
static int runtime_set_failure(struct starsead_runtime *runtime,
    enum starsead_failure_code code, enum starsead_component component,
    const char *message, const struct starsead_child_exit_status *child) {
    char error[128U];
    if (starsead_state_set_failure(runtime->state, code, component, message,
        ...
        starsead_state_set_phase(runtime->state, STARSEAD_PHASE_FAILED) != STARSEAD_STATE_OK ||
        runtime_save(runtime) != 0) return STARSEAD_CONFIG_IO;
    return 0;
}
...
int starsead_runtime_accept_delta(
    ...
    if (runtime == NULL || delta == NULL || (delta->flags & ~known) != 0U ||
        (((delta->flags & STARSEAD_DELTA_STOP_REQUESTED) != 0U) !=
            (delta->stop_reason != STARSEAD_LIFECYCLE_REASON_NONE)) ||
        (((delta->flags & STARSEAD_DELTA_CHILD_EXITED) != 0U) != delta->has_child_exit) ||
        (((delta->flags & STARSEAD_DELTA_RULES_CHANGED) != 0U) != delta->has_rules_summary) ||
        (((delta->flags & STARSEAD_DELTA_FATAL) != 0U) != delta->has_error)) {
        return STARSEAD_CONFIG_INVALID;
    }
    runtime_delta_merge(&runtime->pending_delta, delta);
    if ((delta->flags & STARSEAD_DELTA_STOP_REQUESTED) != 0U &&
        starsead_lifecycle_request_stop(&runtime->lifecycle, delta->stop_reason) ==
            STARSEAD_CONFIG_INVALID) return STARSEAD_CONFIG_INVALID;
    if ((delta->flags & STARSEAD_DELTA_CHILD_EXITED) != 0U) {
        int status = delta->child_exit.has_exit_code ? delta->child_exit.exit_code :
            128 + delta->child_exit.signal_number;
        if (starsead_lifecycle_on_child_exit(
                &runtime->lifecycle, delta->child_role, status) == STARSEAD_CONFIG_INVALID) {
            return STARSEAD_CONFIG_INVALID;
        }
    }
    if ((delta->flags & STARSEAD_DELTA_FATAL) != 0U &&
        starsead_lifecycle_request_stop(
            &runtime->lifecycle, STARSEAD_LIFECYCLE_REASON_RUNTIME_FAILED) ==
            STARSEAD_CONFIG_INVALID) return STARSEAD_CONFIG_INVALID;
    return 0;
}
...
static int runtime_process_delta(
    struct starsead_runtime *runtime, const struct starsead_runtime_delta *delta) {
    struct starsead_runtime_delta terminal = *delta;
    terminal.flags &= STARSEAD_DELTA_STOP_REQUESTED |
        STARSEAD_DELTA_CHILD_EXITED | STARSEAD_DELTA_FATAL;
    if ((terminal.flags & STARSEAD_DELTA_STOP_REQUESTED) == 0U) {
        terminal.stop_reason = STARSEAD_LIFECYCLE_REASON_NONE;
    }
    if ((terminal.flags & STARSEAD_DELTA_CHILD_EXITED) == 0U) {
        terminal.has_child_exit = false;
        terminal.child_role = STARSEAD_CHILD_CORE;
    ...
    if ((terminal.flags & STARSEAD_DELTA_FATAL) == 0U) {
        terminal.has_error = false;
        terminal.error_code = STARSEAD_FAILURE_START_FAILED;
        terminal.error_component = STARSEAD_COMPONENT_RUNTIME;
    ...
    if ((delta->flags & STARSEAD_DELTA_RULES_CHANGED) != 0U) {
        if (runtime_commit_rules(runtime, true, delta->rules_generation,
                delta->rule_categories, true) != 0) return STARSEAD_CONFIG_IO;
    }
    if ((delta->flags & STARSEAD_DELTA_RECONCILE_DUE) != 0U) {
...
static enum starsead_control_event_type runtime_failure_event(
    enum starsead_lifecycle_reason reason) {
    if (reason == STARSEAD_LIFECYCLE_REASON_CORE_EXITED) {
        return STARSEAD_CONTROL_EVENT_CORE_EXITED;
    }
    if (reason == STARSEAD_LIFECYCLE_REASON_HELPER_EXITED) {
        return STARSEAD_CONTROL_EVENT_HELPER_FAILED;
    }
    return STARSEAD_CONTROL_EVENT_FAILED;
}

static enum starsead_component runtime_start_failure_component(const char *stage) {
    if (stage == NULL) return STARSEAD_COMPONENT_RUNTIME;
    if (strcmp(stage, "start_core") == 0 || strcmp(stage, "wait_core") == 0) {
        return STARSEAD_COMPONENT_CORE;
    }
    if (strcmp(stage, "start_helper") == 0 || strcmp(stage, "wait_helper") == 0) {
        return STARSEAD_COMPONENT_HELPER;
    }
    if (strcmp(stage, "start_matcher") == 0) return STARSEAD_COMPONENT_MATCHER;
    if (strcmp(stage, "open_network") == 0) return STARSEAD_COMPONENT_NETWORK;
    if (strcmp(stage, "apply_rules") == 0 || strcmp(stage, "verify") == 0) {
        return STARSEAD_COMPONENT_RULES;
    }
    if (strcmp(stage, "recover") == 0) return STARSEAD_COMPONENT_STATE;
    return STARSEAD_COMPONENT_RUNTIME;
}

static int runtime_start_failure_detail(
    int result, const char *stage, struct starsead_runtime_delta *delta) {
    if (delta == NULL) return STARSEAD_CONFIG_INVALID;
    starsead_runtime_delta_init(delta);
    delta->has_error = true;
    delta->error_component = runtime_start_failure_component(stage);
    if (result == STARSEAD_RUNTIME_EFFECT_READINESS_TIMEOUT) {
        delta->error_code = STARSEAD_FAILURE_READINESS_TIMEOUT;
        (void)snprintf(delta->error_message, sizeof(delta->error_message), "%s",
            delta->error_component == STARSEAD_COMPONENT_HELPER
                ? "required helper readiness timed out" : "required core readiness timed out");
        return 0;
    }
    delta->error_code = STARSEAD_FAILURE_START_FAILED;
...
int starsead_test_capability_path_search_result(bool found, bool valid_input) {
    return system_capability_path_search_result(found, valid_input);
}

int starsead_test_capability_inspect_error(int error_number) {
    return system_capability_inspect_error(error_number);
}
#endif

static int runtime_record_terminal_failure(struct starsead_runtime *runtime,
    enum starsead_lifecycle_reason reason, const struct starsead_runtime_delta *delta) {
    if (reason == STARSEAD_LIFECYCLE_REASON_CONTROL_STOP ||
        reason == STARSEAD_LIFECYCLE_REASON_SIGTERM ||
        reason == STARSEAD_LIFECYCLE_REASON_SIGINT) return 0;
    enum starsead_failure_code code = STARSEAD_FAILURE_START_FAILED;
    enum starsead_component component = STARSEAD_COMPONENT_RUNTIME;
    const char *message = "runtime startup failed";
    const struct starsead_child_exit_status *child = NULL;
    if (reason == STARSEAD_LIFECYCLE_REASON_CORE_EXITED ||
        reason == STARSEAD_LIFECYCLE_REASON_HELPER_EXITED) {
        code = STARSEAD_FAILURE_CHILD_EXITED;
        component = reason == STARSEAD_LIFECYCLE_REASON_CORE_EXITED
            ? STARSEAD_COMPONENT_CORE : STARSEAD_COMPONENT_HELPER;
        message = reason == STARSEAD_LIFECYCLE_REASON_CORE_EXITED
            ? "required core exited" : "required helper exited";
        if (delta != NULL && delta->has_child_exit) child = &delta->child_exit;
    } else if ((reason == STARSEAD_LIFECYCLE_REASON_RUNTIME_FAILED ||
            reason == STARSEAD_LIFECYCLE_REASON_START_FAILED) &&
        delta != NULL && delta->has_error && delta->error_code >= 0 &&
        delta->error_code < STARSEAD_FAILURE_CODE_COUNT &&
        delta->error_component >= 0 && delta->error_component < STARSEAD_COMPONENT_COUNT &&
...
static int runtime_finish(struct starsead_runtime *runtime,
    enum starsead_lifecycle_reason reason, const struct starsead_runtime_delta *delta) {
    bool abnormal = reason != STARSEAD_LIFECYCLE_REASON_CONTROL_STOP &&
        reason != STARSEAD_LIFECYCLE_REASON_SIGTERM &&
        reason != STARSEAD_LIFECYCLE_REASON_SIGINT;
    ...
    if (runtime_set_phase(runtime, STARSEAD_PHASE_STOPPING) != 0 ||
        runtime_publish_event(runtime, STARSEAD_CONTROL_EVENT_STOPPING) != 0) abnormal = true;
    int stopped = starsead_lifecycle_stop(&runtime->lifecycle);
    char error[128U];
    if (stopped == 0 && starsead_state_mark_stopped(
            runtime->state, error, sizeof(error)) == STARSEAD_STATE_OK &&
        runtime_save(runtime) == 0) {
        if (runtime_publish_event(runtime, STARSEAD_CONTROL_EVENT_STOPPED) != 0) abnormal = true;
        return abnormal ? 1 : 0;
    }
    (void)runtime_set_failure(runtime, STARSEAD_FAILURE_STOP_FAILED,
        STARSEAD_COMPONENT_RUNTIME, "critical cleanup incomplete", NULL);
    (void)runtime_publish_event(runtime, STARSEAD_CONTROL_EVENT_FAILED);
    return 1;
}

bool starsead_runtime_remove_tc_before_helper_stop(enum starsead_mode mode) {
    return mode == STARSEAD_MODE_BPF2SOCKS;
}

int starsead_runtime_supervise(struct starsead_runtime *runtime,
    const struct starsead_config *config, struct starsead_state_document *state,
    ...
    if (runtime == NULL || config == NULL || state == NULL || live == NULL ||
        ...
        !runtime_effects_valid(effects, config)) return STARSEAD_CONFIG_INVALID;
    ...
    starsead_runtime_delta_init(&runtime->pending_delta);
    struct starsead_lifecycle_options options = {
        .has_helper = config->helper.type != STARSEAD_HELPER_NONE,
        ...
        .core_managed_traffic = starsead_mode_core_managed(config->mode),
    };
    int started = starsead_lifecycle_start(
        &runtime->lifecycle, &runtime_lifecycle_backend, runtime, &options);
    if (started != 0) {
        enum starsead_lifecycle_reason reason = atomic_load_explicit(
            &runtime->lifecycle.terminal_reason, memory_order_acquire);
        if (reason == STARSEAD_LIFECYCLE_REASON_NONE) reason = STARSEAD_LIFECYCLE_REASON_START_FAILED;
        struct starsead_runtime_delta startup_failure;
    ...
    for (;;) {
        enum starsead_lifecycle_reason reason = atomic_load_explicit(
            &runtime->lifecycle.terminal_reason, memory_order_acquire);
        if (reason != STARSEAD_LIFECYCLE_REASON_NONE) {
            result = runtime_finish(runtime, reason, &runtime->pending_delta);
            break;
        }
        bool processed = false;
        if (runtime_process_deferred_delta(runtime, &processed) != 0) {
            struct starsead_runtime_delta fatal;
            starsead_runtime_delta_init(&fatal);
            fatal.flags = STARSEAD_DELTA_FATAL;
            fatal.has_error = true;
            fatal.error_code = STARSEAD_FAILURE_IO_ERROR;
            fatal.error_component = STARSEAD_COMPONENT_NETWORK;
            (void)snprintf(fatal.error_message, sizeof(fatal.error_message), "%s",
                "deferred reconciliation failed");
            (void)starsead_runtime_accept_delta(runtime, &fatal);
            continue;
        }
        if (processed) continue;
        struct starsead_runtime_delta delta;
        if (starsead_runtime_pump_once(runtime, NULL, &delta) != 0 ||
            runtime_process_delta(runtime, &delta) != 0) {
            starsead_runtime_delta_init(&delta);
            delta.flags = STARSEAD_DELTA_FATAL;
            delta.has_error = true;
            delta.error_code = STARSEAD_FAILURE_IO_ERROR;
            delta.error_component = STARSEAD_COMPONENT_RUNTIME;
            (void)snprintf(delta.error_message, sizeof(delta.error_message), "%s",
                "reactor dispatch failed");
            (void)starsead_runtime_accept_delta(runtime, &delta);
        }
...
static int system_action_post_setup(
    const struct starsead_child_setup_stream *setup,
    bool reaped,
    const struct starsead_child_exit_status *status,
    int *exit_status) {
    if (setup == NULL || status == NULL || exit_status == NULL ||
        !setup->complete || setup->fatal) return STARSEAD_CONFIG_INVALID;
    if (!reaped) return 0;
    if (!status->has_exit_code || status->has_signal) return STARSEAD_CONFIG_INVALID;
    *exit_status = status->exit_code;
    return 1;
}

#define STARSEAD_ACTION_IDENTITY_RETRY_MILLISECONDS 10U
...
static int system_action_post_identity(
    ...
    const struct starsead_child_setup_stream *setup,
    bool reaped, const struct starsead_child_exit_status *status,
    int *exit_status) {
    if (identity_result == 0) return 0;
    if (reap_result == 0) return STARSEAD_CONFIG_NOT_READY;
    if (reap_result < 0) return STARSEAD_CONFIG_INVALID;
    int post_setup = system_action_post_setup(setup, reaped, status, exit_status);
    return post_setup > 0 ? post_setup : STARSEAD_CONFIG_INVALID;
}

static bool system_action_source_active_state(
    bool spawned, bool reaped, enum starsead_poll_source_kind kind) {
    (void)reaped;
    return spawned && kind >= STARSEAD_POLL_PROCESS_EXEC_ERROR &&
        kind <= STARSEAD_POLL_PROCESS_PIDFD;
}

static bool system_action_setup_wait_done_state(
    const struct starsead_child_setup_stream *setup, bool reaped) {
    (void)reaped;
...
#if defined(STARSEAD_TESTING)
bool starsead_test_action_identity_wait_can_retry(
    bool cancelled, int64_t now_milliseconds, int64_t deadline_milliseconds) {
...
bool starsead_test_action_io_drained(
    bool reaped, bool owns_setup, bool owns_stdout, bool owns_stderr) {
...
static bool runtime_event_is_final(enum starsead_control_event_type type) {
    return type == STARSEAD_CONTROL_EVENT_STOPPED ||
        type == STARSEAD_CONTROL_EVENT_FAILED;
}
...
#if defined(STARSEAD_TESTING)
bool starsead_test_runtime_event_is_final(enum starsead_control_event_type type) {
    return runtime_event_is_final(type);
}

bool starsead_test_action_should_cancel(
    bool stop_requested, bool cleanup_in_progress) {
    return action_should_cancel(stop_requested, cleanup_in_progress);
}

bool starsead_test_cycle_failure_requires_shutdown(
    bool service_control_enabled, bool cleanup_complete) {
    return cycle_failure_requires_shutdown(
        service_control_enabled, cleanup_complete);
}

bool starsead_test_admission_reconcile_ready(
    bool listener_acquired, bool action_reactor_ready) {
...
struct system_rule_command_batch {
    ...
    struct system_text_batch ip[STARSEAD_IP_FAMILY_COUNT];
    bool ip_started;
    bool active;
};
...
struct system_rule_snapshot {
    struct system_rule_view xtables[STARSEAD_IP_FAMILY_COUNT][STARSEAD_IP_TABLE_COUNT];
    struct system_rule_view ip_rules[STARSEAD_IP_FAMILY_COUNT];
    struct system_rule_view ip_routes[STARSEAD_IP_FAMILY_COUNT];
    enum system_rule_snapshot_phase phase;
};

struct starsead_system_supervisor {
    struct starsead_loaded_config loaded_config;
    struct starsead_logger logger;
    struct starsead_state_store store;
    struct starsead_state_document state;
    struct starsead_control_live_context live;
    struct starsead_control_server *control;
    struct starsead_runtime *runtime;
    int signal_fd;
    ...
    struct starsead_child_setup_stream core_setup;
    struct starsead_child_identity core_identity;
    bool core_spec_ready;
    bool core_spawned;
    bool core_identity_ready;
    bool core_reaped;
    struct starsead_child_exit_status core_exit;
    struct starsead_helper_launch helper_launch;
    struct starsead_child_process helper_process;
    struct starsead_child_setup_stream helper_setup;
    struct starsead_child_identity helper_identity;
    bool helper_launch_ready;
    bool helper_spawned;
    bool helper_identity_ready;
    bool helper_reaped;
    struct starsead_child_exit_status helper_exit;
    struct starsead_readiness_tracker helper_readiness;
    int helper_readiness_result;
    struct starsead_tun_compat_state tun_compatibility;
    struct starsead_matcher_launch matcher_launch;
    struct starsead_matcher_pin_plan matcher_pin_plan;
    struct starsead_matcher_verification matcher_verification;
    bool matcher_launch_ready;
    bool matcher_plan_ready;
    bool matcher_verified;
    struct starsead_b2s_pin_plan b2s_pin_plan;
    struct starsead_b2s_verification b2s_verification;
    bool b2s_plan_ready;
    bool b2s_verified;
    int tc_netlink_fd;
    ...
    struct starsead_tc_filter_expectation tc_filter_netlink_expectation;
    enum starsead_rules_slot_state tc_filter_netlink_state;
    enum starsead_rules_slot_state tc_qdisc_netlink_state;
    enum starsead_pin_batch_kind active_pin_batch;
    bool pin_batch_active;
    struct starsead_child_process action_process;
    struct starsead_child_setup_stream action_setup;
    struct starsead_child_identity action_identity;
    bool action_spawned;
    bool action_identity_ready;
    bool action_reaped;
    struct starsead_child_exit_status action_exit;
    ...
    struct starsead_system_process_context process_context;
    struct starsead_readiness_backend readiness_backend;
    struct starsead_stop_backend stop_backend;
    struct starsead_readiness_tracker readiness;
    int readiness_result;
    struct starsead_stop_coordinator stop_coordinator;
    int stop_result;
    struct starsead_network_runtime network;
    bool network_opened;
    struct starsead_network_effect_sink network_effect_sink;
    struct starsead_rules_runtime rules_runtime;
    struct starsead_rules_backend rules_backend;
    bool rules_initialized;
    bool rules_verified;
    bool local_address_snapshot_active;
    struct starsead_address_set local_ipv4_snapshot;
    struct starsead_address_set local_ipv6_snapshot;
    bool cleanup_in_progress;
    bool owned_cleanup_in_progress;
};

static int system_collect_local_address_set(
    struct starsead_system_supervisor *, int, struct starsead_address_set *);

static const char *system_pin_path(
    struct starsead_system_supervisor *, enum starsead_pin_id);
static uint64_t system_verified_pin_id(
    const struct starsead_system_supervisor *, enum starsead_pin_id);
static const struct starsead_b2s_verified_pin *system_b2s_verified_pin(
    const struct starsead_system_supervisor *, enum starsead_pin_id);
static int system_tc_netlink_dispatch(struct starsead_system_supervisor *, short);
...
static int system_runtime_local_time(void *opaque, struct starsead_local_time *output) {
    (void)opaque;
    ...
    *output = (struct starsead_local_time){
...
static int system_runtime_add_source(struct starsead_poll_builder *builder, int fd,
    short events, enum starsead_poll_source_kind kind, uint32_t slot, uint64_t generation) {
    if (fd < 0) return 0;
    struct starsead_poll_source source = {
        .fd = fd,
        .events = events,
        .kind = kind,
        .slot = slot,
    ...
    return starsead_poll_builder_add(builder, &source);
}

static void system_runtime_min_deadline(
    struct starsead_deadline *deadline, uint64_t candidate) {
    if (candidate > (uint64_t)INT64_MAX) return;
    struct starsead_deadline next = {
        .armed = true,
        .monotonic_milliseconds = (int64_t)candidate,
    };
    starsead_deadline_min(deadline, &next);
}
...
static int system_service_timer_arm(struct starsead_system_supervisor *system) {
    ...
    const struct starsead_service_control_config *control =
    ...
    if (control->enabled && control->schedule.enabled) {
        ...
        bool has_start = now != (time_t)-1 &&
            starsead_cron_next(&control->schedule.start, now, &next_start) == 0;
        bool has_stop = now != (time_t)-1 &&
            starsead_cron_next(&control->schedule.stop, now, &next_stop) == 0;
...
static int system_service_reconcile_time(struct starsead_system_supervisor *system) {
    time_t now = time(NULL);
    if (now == (time_t)-1) return -1;
    system_service_apply_action(system,
        starsead_service_control_reconcile_time(&system->service_control, now));
    return system_service_timer_arm(system);
}

static int system_service_timer_dispatch(struct starsead_system_supervisor *system) {
...
static int system_service_wifi_dispatch(struct starsead_system_supervisor *system) {
    enum starsead_wifi_transition transition = STARSEAD_WIFI_TRANSITION_BASELINE_DISCONNECTED;
    struct starsead_wifi_identity identity;
    bool has_transition = false;
    char error[128U];
    if (starsead_wifi_monitor_handle(&system->wifi_monitor, &transition,
            &identity, &has_transition, error, sizeof(error)) != 0) return -1;
    if (has_transition) {
        system_service_apply_action(system,
            starsead_service_control_on_wifi(
...
static int system_service_wifi_reconcile(
    struct starsead_system_supervisor *system, uint64_t now) {
    enum starsead_wifi_transition transition = STARSEAD_WIFI_TRANSITION_BASELINE_DISCONNECTED;
    struct starsead_wifi_identity identity;
    bool has_transition = false;
    char error[128U];
    if (starsead_wifi_monitor_take_reconcile(&system->wifi_monitor, now,
    ...
    if (system->action_spawned) {
        ...
        if (!system->action_reaped && system->action_process.owns_pidfd &&
            system_runtime_add_source(builder, system->action_process.pidfd, POLLIN,
                STARSEAD_POLL_PROCESS_PIDFD, 2U, generation) != 0) return -1;
    }
    if (system->control != NULL) {
        struct starsead_control_interest interests[STARSEAD_CONTROL_MAX_CLIENTS + 1U];
        size_t count = starsead_control_server_interests(
            system->control, interests, sizeof(interests) / sizeof(interests[0]));
        if (count > sizeof(interests) / sizeof(interests[0])) return -1;
        int listener = starsead_control_server_listener_fd(system->control);
        for (size_t index = 0U; index < count; ++index) {
            short events = (short)((interests[index].readable ? POLLIN : 0) |
                (interests[index].writable ? POLLOUT : 0));
            enum starsead_poll_source_kind kind = interests[index].fd == listener
                ? STARSEAD_POLL_CONTROL_LISTENER : STARSEAD_POLL_CONTROL_CLIENT;
        ...
        if (starsead_control_server_next_deadline(system->control, &control_deadline)) {
...
static int system_runtime_wait(void *opaque, const struct starsead_poll_builder *builder,
    int64_t timeout_milliseconds, short *ready) {
    (void)opaque;
    struct pollfd descriptors[STARSEAD_MAX_POLL_SOURCES];
    for (size_t index = 0U; index < builder->count; ++index) {
    ...
    if (result < 0) return errno == EINTR
        ? STARSEAD_REACTOR_WAIT_INTERRUPTED : STARSEAD_REACTOR_WAIT_ERROR;
    for (size_t index = 0U; index < builder->count; ++index) ready[index] = descriptors[index].revents;
    return result;
}

static void system_runtime_fatal(struct starsead_runtime_delta *delta,
    enum starsead_component component, const char *message) {
    if ((delta->flags & STARSEAD_DELTA_FATAL) != 0U) return;
    delta->flags |= STARSEAD_DELTA_FATAL;
    delta->has_error = true;
    delta->error_code = STARSEAD_FAILURE_IO_ERROR;
...
static void system_runtime_note_exit(struct starsead_system_supervisor *system,
    enum starsead_child_role role, int status, struct starsead_runtime_delta *delta) {
    bool *reaped = role == STARSEAD_CHILD_CORE ? &system->core_reaped : &system->helper_reaped;
    struct starsead_child_exit_status *exit_status = role == STARSEAD_CHILD_CORE
        ? &system->core_exit : &system->helper_exit;
    if (*reaped) return;
    *reaped = true;
    if (starsead_child_exit_status_from_wait(status, exit_status) != 0) {
    ...
    delta->flags |= STARSEAD_DELTA_CHILD_EXITED;
...
static void system_runtime_note_action_exit(
    struct starsead_system_supervisor *system, int status) {
    if (system->action_reaped) return;
    system->action_reaped = true;
    if (starsead_child_exit_status_from_wait(status, &system->action_exit) != 0) {
        memset(&system->action_exit, 0, sizeof(system->action_exit));
    }
}

static int system_runtime_reap_children(
    struct starsead_system_supervisor *system, struct starsead_runtime_delta *delta) {
    for (;;) {
        int status = 0;
        ...
        if (system->core_spawned && pid == (pid_t)system->core_process.pid) {
            system_runtime_note_exit(system, STARSEAD_CHILD_CORE, status, delta);
        } else if (system->helper_spawned && pid == (pid_t)system->helper_process.pid) {
            system_runtime_note_exit(system, STARSEAD_CHILD_HELPER, status, delta);
        ...
        if (info.ssi_signo == SIGTERM || info.ssi_signo == SIGINT) {
            ...
            if (system->service_running &&
                ...
                delta->stop_reason = info.ssi_signo == SIGTERM
                    ? STARSEAD_LIFECYCLE_REASON_SIGTERM : STARSEAD_LIFECYCLE_REASON_SIGINT;
            }
...
static int system_runtime_dispatch_setup(struct starsead_system_supervisor *system,
    uint32_t slot, struct starsead_runtime_delta *delta) {
    struct starsead_child_process *process = slot == 0U ? &system->core_process :
        slot == 1U ? &system->helper_process : &system->action_process;
    struct starsead_child_setup_stream *setup = slot == 0U ? &system->core_setup :
        slot == 1U ? &system->helper_setup : &system->action_setup;
    enum starsead_component component = slot == 0U
        ? STARSEAD_COMPONENT_CORE : slot == 1U
            ? STARSEAD_COMPONENT_HELPER : STARSEAD_COMPONENT_RUNTIME;
    ...
    for (;;) {
        ...
        if (count < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) return 0;
        bool eof = count == 0;
        if (count < 0 || starsead_child_setup_stream_feed(setup,
                bytes, count > 0 ? (size_t)count : 0U, eof) < STARSEAD_CHILD_SETUP_PENDING) {
            if (slot != 2U) {
...
static int system_runtime_dispatch_log(struct starsead_system_supervisor *system,
    uint32_t slot, enum starsead_log_stream stream, struct starsead_runtime_delta *delta) {
    (void)delta;
    struct starsead_child_process *process = slot == 0U ? &system->core_process :
        slot == 1U ? &system->helper_process : &system->action_process;
    int *fd = stream == STARSEAD_LOG_STREAM_STDOUT
        ? &process->stdout_fd : &process->stderr_fd;
    bool *owned = stream == STARSEAD_LOG_STREAM_STDOUT
    ...
    for (;;) {
        ...
        if (count < 0 && errno == EINTR) continue;
        if (count < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) return 0;
        if (count > 0 && slot == 2U && stream == STARSEAD_LOG_STREAM_STDOUT &&
...
static int system_runtime_dispatch(void *opaque, const struct starsead_poll_source *source,
    short ready, struct starsead_runtime_delta *delta) {
    struct starsead_system_supervisor *system = opaque;
    if (source->kind >= STARSEAD_POLL_PROCESS_EXEC_ERROR &&
        source->kind <= STARSEAD_POLL_PROCESS_PIDFD) {
        struct starsead_child_process *process = source->slot == 0U ? &system->core_process :
        ...
        if (!active || !starsead_process_poll_source_matches(process, source)) {
            system_runtime_fatal(delta, STARSEAD_COMPONENT_RUNTIME,
                "stale child poll source");
            return 0;
        }
    }
    if (source->kind == STARSEAD_POLL_SIGNAL) {
        if (system_runtime_dispatch_signal(system, delta) != 0) {
            system_runtime_fatal(delta, STARSEAD_COMPONENT_RUNTIME, "signal drain failed");
        }
    } else if (source->kind == STARSEAD_POLL_PROCESS_EXEC_ERROR) {
        (void)system_runtime_dispatch_setup(system, source->slot, delta);
    } else if (source->kind == STARSEAD_POLL_PROCESS_STDOUT) {
        (void)system_runtime_dispatch_log(
            system, source->slot, STARSEAD_LOG_STREAM_STDOUT, delta);
    } else if (source->kind == STARSEAD_POLL_PROCESS_STDERR) {
        (void)system_runtime_dispatch_log(
            system, source->slot, STARSEAD_LOG_STREAM_STDERR, delta);
    } else if (source->kind == STARSEAD_POLL_PROCESS_PIDFD && !system->stopping_children) {
        if (system_runtime_reap_children(system, delta) != 0) {
            system_runtime_fatal(delta, STARSEAD_COMPONENT_CORE, "core reap failed");
        }
    } else if ((source->kind == STARSEAD_POLL_CONTROL_LISTENER ||
            source->kind == STARSEAD_POLL_CONTROL_CLIENT) && system->control != NULL) {
        int64_t now = 0;
        if (system_runtime_clock(system, &now) != 0 ||
            starsead_control_server_dispatch(system->control, source->fd,
                (ready & POLLIN) != 0, (ready & POLLOUT) != 0,
                (ready & (POLLERR | POLLHUP | POLLNVAL)) != 0, (uint64_t)now) != 0) {
            system_runtime_fatal(delta, STARSEAD_COMPONENT_CONTROL, "control dispatch failed");
        }
    } else if (source->kind == STARSEAD_POLL_NETWORK && system->network_opened) {
        int64_t now = 0;
        char error[128U];
        if (system_runtime_clock(system, &now) != 0 ||
            starsead_network_handle(
                &system->network, (uint64_t)now, error, sizeof(error)) != 0) {
            system_runtime_fatal(delta, STARSEAD_COMPONENT_NETWORK, "network drain failed");
        } else if (starsead_network_has_immediate(&system->network)) {
            delta->flags |= STARSEAD_DELTA_NETWORK_CHANGED;
        }
    } else if (source->kind == STARSEAD_POLL_TC_NETLINK &&
    ...
    } else if (source->kind == STARSEAD_POLL_SERVICE_TIMER) {
    ...
    if (system->stop_requested &&
        ...
        delta->flags |= STARSEAD_DELTA_STOP_REQUESTED;
        delta->stop_reason = STARSEAD_LIFECYCLE_REASON_CONTROL_STOP;
    }
    return 0;
}

static int system_runtime_expire(
    void *opaque, int64_t now, struct starsead_runtime_delta *delta) {
    struct starsead_system_supervisor *system = opaque;
    if (system->control != NULL &&
        starsead_control_server_tick(system->control, (uint64_t)now) != 0) {
        system_runtime_fatal(delta, STARSEAD_COMPONENT_CONTROL, "control deadline failed");
    }
    if (system->logger.opened &&
        starsead_log_flush_expired(&system->logger, (uint64_t)now) != 0) {
        system_runtime_fatal(delta, STARSEAD_COMPONENT_LOG, "log deadline failed");
    }
    uint64_t network_deadline = 0U;
    if (system->network_opened &&
        starsead_network_next_deadline(&system->network, &network_deadline) &&
        ...
        if (starsead_network_take_reconcile(
                &system->network, (uint64_t)now, &batch, &integrity_loss) != 0) {
            system_runtime_fatal(delta, STARSEAD_COMPONENT_NETWORK,
                "network debounce failed");
        } else {
            delta->flags |= STARSEAD_DELTA_RECONCILE_DUE;
            (void)integrity_loss;
        }
    }
    uint64_t wifi_deadline = 0U;
...
static struct starsead_runtime_reactor_backend system_runtime_reactor_backend(void) {
    const struct starsead_runtime_reactor_backend backend = {
...
static int system_runtime_snapshot(
    void *opaque, struct starsead_control_snapshot *snapshot) {
    struct starsead_system_supervisor *system = opaque;
    return starsead_control_snapshot_from_state(&system->state, &system->live, snapshot);
}
...
static int system_accept_pump_delta(struct starsead_system_supervisor *system,
    struct starsead_runtime_delta *delta) {
    if (system == NULL || delta == NULL) return -1;
    return delta->flags == 0U ? 0 :
        starsead_runtime_accept_delta(system->runtime, delta);
}

static int system_pump_condition(struct starsead_system_supervisor *,
    const struct starsead_deadline *, starsead_runtime_predicate);
static int system_pump_condition_periodic(struct starsead_system_supervisor *,
    const struct starsead_deadline *, uint32_t, starsead_runtime_predicate);
static bool system_stop_done(void *);

static int system_effect_save(
    void *opaque, const struct starsead_state_document *state) {
    struct starsead_system_supervisor *system = opaque;
    char error[256U];
    return starsead_state_store_save(&system->store, state, error, sizeof(error));
}

static int system_effect_event(void *opaque, enum starsead_control_event_type type,
    const struct starsead_control_snapshot *snapshot,
    const struct starsead_control_error *details, bool has_details) {
    struct starsead_system_supervisor *system = opaque;
    int64_t now = 0;
...
static int system_reconcile_owned_resources(
    struct starsead_system_supervisor *, char *, size_t);
...
static int system_core_start_failed(
    ...
    const char *detail) {
    struct starsead_core_start_diagnostic diagnostic = {
        .stage = stage,
    ...
    char message[512U];
    if (starsead_core_start_diagnostic_format(
            &diagnostic, message, sizeof(message)) == STARSEAD_LOG_OK) {
        int log_result = starsead_log_line(&system->logger, STARSEAD_LOG_LEVEL_ERROR,
            STARSEAD_COMPONENT_RUNTIME, STARSEAD_LOG_EVENT_DIAGNOSTIC, message);
        if (log_result != STARSEAD_LOG_OK) {
...
static int system_effect_start_core(
    ...
    struct starsead_system_supervisor *system = opaque;
    char error[256U] = {0};
    if (!system->core_spec_ready && starsead_core_process_spec(
        ...
        return system_core_start_failed(system,
            STARSEAD_CORE_START_FAILURE_PROCESS_SPEC,
            error[0] == '\0' ? "process specification failed" : error);
    }
    system->core_spec_ready = true;
    if (starsead_system_process_backends_init(&system->process_context,
            &system->core_spec, NULL, &system->readiness_backend,
            &system->stop_backend) != 0) {
        return system_core_start_failed(system,
            STARSEAD_CORE_START_FAILURE_BACKEND_INIT,
            "process backend initialization failed");
    }
    if (starsead_readiness_preflight(&system->loaded_config.config,
            STARSEAD_CHILD_CORE, &system->readiness_backend) != 0) {
        return system_core_start_failed(system,
            STARSEAD_CORE_START_FAILURE_READINESS_PREFLIGHT,
            "readiness preflight failed");
    }
    if (starsead_process_spawn_system(
            &system->core_spec, &system->core_process, error, sizeof(error)) != 0) {
        return system_core_start_failed(system, STARSEAD_CORE_START_FAILURE_SPAWN,
            error[0] == '\0' ? "process spawn failed" : error);
    }
    system->core_spawned = true;
    starsead_child_setup_stream_init(&system->core_setup);
    int64_t now = 0;
    if (system_runtime_clock(system, &now) != 0 ||
        (uint64_t)now > UINT64_MAX - system->loaded_config.config.readiness_timeout_milliseconds) {
        return system_core_start_failed(system, STARSEAD_CORE_START_FAILURE_CLOCK,
            "setup deadline clock failed");
    }
    struct starsead_deadline deadline = {
        .armed = true,
    ...
    if (pumped != 0) {
        return system_core_start_failed(system, STARSEAD_CORE_START_FAILURE_SETUP_WAIT,
            "setup status wait failed");
    }
    if (!system->core_setup.complete || system->core_setup.fatal || system->core_reaped) {
        return system_core_start_failed(system, STARSEAD_CORE_START_FAILURE_SETUP_RESULT,
            "child setup failed");
    }
    enum starsead_child_type type = system->loaded_config.config.core_type == STARSEAD_CORE_XRAY
        ? STARSEAD_CHILD_TYPE_XRAY :
        system->loaded_config.config.core_type == STARSEAD_CORE_SING_BOX
            ? STARSEAD_CHILD_TYPE_SING_BOX : STARSEAD_CHILD_TYPE_MIHOMO;
    if (starsead_process_identity_read(starsead_system_process_identity_backend(),
            system->core_process.pid, STARSEAD_CHILD_CORE, type, &system->core_spec,
            &system->core_identity, error, sizeof(error)) != 0) {
        return system_core_start_failed(system, STARSEAD_CORE_START_FAILURE_IDENTITY,
...
static int system_effect_wait_core(void *opaque) {
    ...
    int64_t now = 0;
    if (system_runtime_clock(system, &now) != 0 ||
        starsead_system_process_backends_init(&system->process_context,
            &system->core_spec, NULL, &system->readiness_backend, &system->stop_backend) != 0 ||
        starsead_readiness_init(&system->loaded_config.config, STARSEAD_CHILD_CORE,
    ...
    int pumped = system_pump_condition_periodic(system, &deadline,
        STARSEAD_READINESS_POLL_INTERVAL_MILLIS, system_core_readiness_done);
    if (pumped == 0 && system->readiness_result == STARSEAD_READINESS_READY) return 0;
    return system->readiness_result == STARSEAD_READINESS_TIMEOUT
        ? STARSEAD_RUNTIME_EFFECT_READINESS_TIMEOUT : -1;
}

static bool system_helper_setup_done(void *opaque);

static const struct starsead_tun_compat_paths system_tun_compatibility_paths = {
    .tun_device = "/dev/tun",
...
static int system_tun_compatibility_log(
    struct starsead_system_supervisor *system, const char *operation, const char *detail) {
    char message[384U];
    ...
    if (written > 0 && (size_t)written < sizeof(message)) {
        (void)starsead_log_line(&system->logger, STARSEAD_LOG_LEVEL_ERROR,
            STARSEAD_COMPONENT_HELPER, STARSEAD_LOG_EVENT_DIAGNOSTIC, message);
    }
    return -1;
}

static int system_tun_compatibility_prepare(struct starsead_system_supervisor *system) {
    char error[256U];
    if (starsead_tun_compat_prepare(&system_tun_compatibility_paths,
            &system->tun_compatibility, error, sizeof(error)) == 0) return 0;
    (void)system_tun_compatibility_log(system, "prepare", error);
    if (starsead_tun_compat_cleanup(&system_tun_compatibility_paths,
...
static int system_tun_compatibility_cleanup(struct starsead_system_supervisor *system) {
    char error[256U];
    if (starsead_tun_compat_cleanup(&system_tun_compatibility_paths,
            &system->tun_compatibility, error, sizeof(error)) == 0) return 0;
...
static int system_start_helper_process(
    ...
    if (starsead_system_process_backends_init(&system->process_context,
        ...
        starsead_readiness_preflight(&system->loaded_config.config,
            STARSEAD_CHILD_HELPER, &system->readiness_backend) != 0) return -1;
    if (starsead_process_spawn_system(&system->helper_launch.process,
            &system->helper_process, error, sizeof(error)) != 0) return -1;
    system->helper_spawned = true;
    starsead_child_setup_stream_init(&system->helper_setup);
    int64_t now = 0;
    ...
    const struct starsead_anonymous_file_backend *anonymous =
        starsead_system_anonymous_file_backend();
    if (starsead_anonymous_file_close(anonymous, &system->helper_launch.config_file) != 0 ||
        starsead_anonymous_file_close(anonymous, &system->helper_launch.direct_ipv4_file) != 0 ||
        starsead_anonymous_file_close(anonymous, &system->helper_launch.direct_ipv6_file) != 0) {
        return -1;
    }
    enum starsead_child_type type = system->loaded_config.config.helper.type ==
        STARSEAD_HELPER_HEV_SOCKS5_TUNNEL
            ? STARSEAD_CHILD_TYPE_HEV_SOCKS5_TUNNEL : STARSEAD_CHILD_TYPE_BPF2SOCKS;
    if (starsead_process_identity_read(starsead_system_process_identity_backend(),
            system->helper_process.pid, STARSEAD_CHILD_HELPER, type,
...
static bool system_helper_readiness_done(void *opaque) {
    ...
    int64_t now = 0;
    if (system_runtime_clock(system, &now) != 0) {
        system->helper_readiness_result = STARSEAD_READINESS_IO;
        return true;
    }
    bool stop = atomic_load_explicit(
        &system->runtime->lifecycle.stop_was_requested, memory_order_acquire);
    system->helper_readiness_result = starsead_readiness_poll(
        &system->loaded_config.config, &system->helper_readiness, &system->helper_identity,
        &system->readiness_backend, (uint64_t)now, stop);
    return system->helper_readiness_result != STARSEAD_READINESS_PENDING;
}
...
static int system_effect_wait_helper(void *opaque) {
    ...
    if (system_runtime_clock(system, &now) != 0 ||
        starsead_system_process_backends_init(&system->process_context,
            &system->core_spec, &system->helper_launch.process,
            &system->readiness_backend, &system->stop_backend) != 0 ||
        starsead_readiness_init(&system->loaded_config.config, STARSEAD_CHILD_HELPER,
            (uint64_t)now, &system->readiness_backend, &system->helper_readiness) != 0) goto done;
    {
        ...
        int pumped = system_pump_condition_periodic(system, &deadline,
            STARSEAD_READINESS_POLL_INTERVAL_MILLIS, system_helper_readiness_done);
        if (pumped == 0 && system->helper_readiness_result == STARSEAD_READINESS_READY) {
            result = 0;
        } else if (system->helper_readiness_result == STARSEAD_READINESS_TIMEOUT) {
            result = STARSEAD_RUNTIME_EFFECT_READINESS_TIMEOUT;
        }
    }
...
static bool system_action_exit_done(void *opaque) {
    return ((struct starsead_system_supervisor *)opaque)->action_reaped;
}
...
static int system_pump_condition(struct starsead_system_supervisor *system,
    const struct starsead_deadline *deadline, starsead_runtime_predicate predicate) {
    for (;;) {
        ...
        struct starsead_runtime_delta delta;
        if (starsead_runtime_pump_once(system->runtime, deadline, &delta) != 0) return -1;
        if (system_accept_pump_delta(system, &delta) != 0) return -1;
    }
}

static int system_pump_condition_periodic(struct starsead_system_supervisor *system,
    const struct starsead_deadline *deadline, uint32_t interval,
    starsead_runtime_predicate predicate) {
    ...
    for (;;) {
        ...
        if (system_runtime_clock(system, &now) != 0 ||
            now >= deadline->monotonic_milliseconds) return -1;
        struct starsead_deadline iteration = {.armed = true};
        if (runtime_periodic_deadline(now, deadline->monotonic_milliseconds,
                interval, &iteration.monotonic_milliseconds) != 0) return -1;
        struct starsead_runtime_delta delta;
        if (starsead_runtime_pump_once(system->runtime, &iteration, &delta) != 0) return -1;
        if (system_accept_pump_delta(system, &delta) != 0) return -1;
    }
}

static int system_action_signal_unverified(
    struct starsead_system_supervisor *system, int signal_number) {
...
static int system_action_cleanup(struct starsead_system_supervisor *system,
    ...
    if (system_runtime_clock(system, &now) != 0 || now < 0 ||
        now > INT64_MAX - (int64_t)(STARSEAD_PROCESS_TERM_GRACE_MILLIS +
            STARSEAD_PROCESS_KILL_REAP_MILLIS)) return -1;

    if (system->action_identity_ready) {
        if (starsead_system_process_backends_init(&system->process_context,
                spec, spec, &system->readiness_backend, &system->stop_backend) != 0) return -1;
        starsead_stop_coordinator_init(&system->stop_coordinator);
        system->stopping_children = true;
        system->stop_result = starsead_stop_coordinator_begin(
            &system->stop_coordinator, NULL, &system->action_identity,
            &system->stop_backend, (uint64_t)now);
        if (system->stop_result == STARSEAD_STOP_PENDING) {
            struct starsead_deadline deadline = {
                .armed = true,
            ...
            if (system_pump_condition(system, &deadline, system_stop_done) != 0) {
                system->stop_result = STARSEAD_STOP_FAILED;
            }
        }
        system->stopping_children = false;
        if (system->stop_result != STARSEAD_STOP_COMPLETE) return -1;
        system->action_reaped = true;
    ...
    struct starsead_deadline term_deadline = {
        .armed = true,
        .monotonic_milliseconds = now + STARSEAD_PROCESS_TERM_GRACE_MILLIS,
    };
...
static int system_action_run_spec(struct starsead_system_supervisor *system,
    const struct starsead_process_spec *spec, bool capture_stdout, int *exit_status) {
    ...
    if (starsead_process_spawn_system(spec, &system->action_process,
        ...
        if (written > 0 && (size_t)written < sizeof(message)) {
            (void)starsead_log_line(&system->logger,
                system->owned_cleanup_in_progress ? STARSEAD_LOG_LEVEL_WARNING : STARSEAD_LOG_LEVEL_ERROR,
                STARSEAD_COMPONENT_RUNTIME, STARSEAD_LOG_EVENT_DIAGNOSTIC, message);
        }
        return -1;
    }
    system->action_spawned = true;
    starsead_child_setup_stream_init(&system->action_setup);
    int64_t now = 0;
    failure_phase = "setup-clock";
    if (system_runtime_clock(system, &now) != 0) goto failed;
    struct starsead_deadline setup_deadline = {
        .armed = true,
        .monotonic_milliseconds = now +
            (int64_t)system->loaded_config.config.readiness_timeout_milliseconds,
    };
    ...
    int identity_result = starsead_process_identity_read(
            starsead_system_process_identity_backend(),
            system->action_process.pid, STARSEAD_CHILD_HELPER,
            STARSEAD_CHILD_TYPE_BPF2SOCKS, spec, &system->action_identity,
            error, sizeof(error));
    while (identity_result != 0) {
        ...
        if (post_identity != STARSEAD_CONFIG_NOT_READY ||
    ...
    struct starsead_deadline exit_deadline = {
        .armed = true,
    ...
    *exit_status = system->action_exit.exit_code;
    starsead_child_process_close(&system->action_process);
...
static int system_action_spec(const char *const *argv, struct starsead_process_spec *spec) {
    ...
    if (starsead_process_environment_rebuild(
            (const char *const *)environ, spec) != 0) goto failed;
    for (size_t index = 0U; argv[index] != NULL; ++index) {
        if (starsead_process_argument_add(spec, argv[index]) != 0) goto failed;
    }
    return 0;
failed:
...
static int system_action_run_argv(struct starsead_system_supervisor *system,
...
static void system_rule_batch_destroy(struct system_rule_command_batch *batch) {
    ...
    for (size_t index = 0U; index < STARSEAD_IP_FAMILY_COUNT; ++index) {
...
static int system_action_run_document(
    ...
    char error[128U];
    if (starsead_anonymous_file_create(
            backend, "starsead-rule-batch", &document,
    ...
    starsead_process_spec_destroy(&spec);
    if (starsead_anonymous_file_close(backend, &file) != 0) result = -1;
    return result;
}
...
static int system_rule_batch_flush(
    ...
    size_t command_length = batch->xtables_before_ip.length +
        batch->ip[STARSEAD_IP_FAMILY_IPV4].length +
        batch->ip[STARSEAD_IP_FAMILY_IPV6].length +
        batch->xtables_after_ip.length;
    if (command_length == 0U) return 0;
    struct system_text_batch document = {0};
    if (starsead_rule_batch_document_render(
            batch->xtables_before_ip.bytes, batch->xtables_before_ip.length,
            batch->ip[STARSEAD_IP_FAMILY_IPV4].bytes,
            batch->ip[STARSEAD_IP_FAMILY_IPV4].length,
            batch->ip[STARSEAD_IP_FAMILY_IPV6].bytes,
            batch->ip[STARSEAD_IP_FAMILY_IPV6].length,
...
static int system_capability_find(
    ...
    while (*cursor != '\0') {
        ...
        if (length != 0U && length < STARSEAD_MAX_PATH) {
            ...
            if (written > 0 && (size_t)written < capacity && stat(path, &status) == 0 &&
                S_ISREG(status.st_mode) && starsead_mode_is_executable((uint32_t)status.st_mode)) {
...
static int system_capability_inspect(
    ...
    *executable = *regular && starsead_mode_is_executable((uint32_t)status.st_mode);
    return 0;
}
...
static int system_effect_capability(void *opaque) {
    ...
    char error[256U];
    int prepared = starsead_platform_capability_ensure(
...
static const char *system_xtables_path(enum starsead_ip_family family) {
    return family == STARSEAD_IP_FAMILY_IPV4
        ? "iptables" : family == STARSEAD_IP_FAMILY_IPV6
            ? "ip6tables" : NULL;
}

static const char *system_table_name(enum starsead_ip_table table) {
    static const char *const names[] = {"filter", "nat", "mangle"};
    return table >= STARSEAD_IP_TABLE_FILTER && table < STARSEAD_IP_TABLE_COUNT
        ? names[table] : NULL;
}

static const char *system_builtin_name(enum starsead_builtin_chain chain) {
    static const char *const names[] = {"PREROUTING", "INPUT", "FORWARD", "OUTPUT"};
    return chain >= STARSEAD_BUILTIN_PREROUTING && chain < STARSEAD_BUILTIN_COUNT
        ? names[chain] : NULL;
}

static int system_xtables_run(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, enum starsead_ip_table table,
...
static int system_xtables(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, enum starsead_ip_table table,
    const char *operation, const char *chain, const char *const *arguments,
...
static int system_xtables_zero(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, enum starsead_ip_table table,
    const char *operation, const char *chain, const char *const *arguments,
    ...
    if (system->verify_private_rules) {
        ...
        if (system->rule_snapshot.phase == SYSTEM_RULE_SNAPSHOT_BEFORE_APPLY ||
            ...
            size_t matches = 0U;
            size_t position = 0U;
            if (!view->present || starsead_xtables_rule_output_locate(
    ...
    if (result != 0) {
        ...
        if (written > 0 && (size_t)written < sizeof(message)) {
            (void)starsead_log_line(&system->logger, STARSEAD_LOG_LEVEL_ERROR,
                STARSEAD_COMPONENT_RULES, STARSEAD_LOG_EVENT_DIAGNOSTIC, message);
        }
    }
...
static bool system_cidr_matches_family(
    const char *cidr, enum starsead_ip_family family) {
    return cidr != NULL && ((strchr(cidr, ':') != NULL) ==
        (family == STARSEAD_IP_FAMILY_IPV6));
}

static int system_append_jump(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain, const char *target) {
    const char *arguments[] = {"-j", target};
    return system_xtables_zero(system, family, STARSEAD_IP_TABLE_MANGLE,
        "-A", chain, arguments, 2U);
}

static int system_canonical_cidr(enum starsead_ip_family family,
    ...
    int native_family = family == STARSEAD_IP_FAMILY_IPV4 ? AF_INET : AF_INET6;
    union {
        struct in_addr ipv4;
...
static int system_append_return_cidr(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain, const char *cidr) {
    char canonical[STARSEAD_MAX_CIDR];
...
static int system_append_return_interface(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain, bool input, const char *name) {
    const char *arguments[] = {input ? "-i" : "-o", name, "-j", "RETURN"};
    return system_xtables_zero(system, family, STARSEAD_IP_TABLE_MANGLE,
        "-A", chain, arguments, 4U);
}

static int system_append_uid_return(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain, uint32_t uid) {
    char uid_text[16U];
...
static int system_append_gid_return(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain) {
    const char *arguments[] = {"-m", "owner", "--gid-owner", "3005", "-j", "RETURN"};
    return system_xtables_zero(system, family, STARSEAD_IP_TABLE_MANGLE,
        "-A", chain, arguments, 6U);
}

static int system_append_ipsec_udp_bypass(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain) {
    ...
    return system_xtables_zero(system, family, STARSEAD_IP_TABLE_MANGLE,
        ...
        system_xtables_zero(system, family, STARSEAD_IP_TABLE_MANGLE,
            "-A", chain, nat_t_arguments, 8U) == 0 ? 0 : -1;
}

static int system_append_mark(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain, const char *protocol,
    const char *uid, const char *input_interface, const char *destination,
    const char *bpf_path) {
    const char *arguments[24U];
    size_t count = 0U;
    char canonical_destination[STARSEAD_MAX_CIDR];
    if (destination != NULL) {
...
static int system_append_tproxy(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain, const char *protocol,
    ...
    arguments[count++] = family == STARSEAD_IP_FAMILY_IPV4 ? "0.0.0.0" : "::";
...
static int system_append_dns_mark(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain, bool bypass_core_gid) {
...
static int system_append_dns_tproxy(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain) {
    char port[8U];
    if (snprintf(port, sizeof(port), "%u",
            (unsigned)system->loaded_config.config.transparent_port) <= 0) return -1;
    const char *arguments[] = {"-p", "udp", "-m", "udp", "--dport", "53",
        ...
        family == STARSEAD_IP_FAMILY_IPV4 ? "0.0.0.0" : "::",
        "--tproxy-mark", "0x20000000/0x60000000"};
    return system_xtables_zero(system, family, STARSEAD_IP_TABLE_MANGLE,
        "-A", chain, arguments, sizeof(arguments) / sizeof(arguments[0]));
}

static int system_populate_local_chain(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain) {
    int address_family = family == STARSEAD_IP_FAMILY_IPV4
        ? STARSEAD_ADDRESS_IPV4 : STARSEAD_ADDRESS_IPV6;
    struct starsead_address_set live;
    const struct starsead_address_set *set = NULL;
    if (system->local_address_snapshot_active) {
        set = family == STARSEAD_IP_FAMILY_IPV4
...
static int system_append_policy_output(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *chain) {
    ...
    if (config->matcher.enabled) {
        ...
        enum starsead_pin_id pin = family == STARSEAD_IP_FAMILY_IPV4
...
static int system_populate_common_output_prefix(
    struct starsead_system_supervisor *system, enum starsead_ip_family family,
...
static int system_populate_tproxy_prerouting(
    ...
    if (family == STARSEAD_IP_FAMILY_IPV6 && !config->enable_ipv6) return 0;
    if (system_append_ipsec_udp_bypass(system, family, chain) != 0) return -1;
    if (system_append_local_bypass_interval(
            system, family, chain, local_begin, local_end) != 0) return -1;
    ...
    if (config->matcher.enabled) {
        matcher_path = system_pin_path(system, family == STARSEAD_IP_FAMILY_IPV4
...
static int system_populate_tproxy_output(
    ...
    const char *chain, const char *local_begin, const char *local_end) {
    const struct starsead_config *config = &system->loaded_config.config;
    if (family == STARSEAD_IP_FAMILY_IPV6 && !config->enable_ipv6) {
        return config->enable_local_dns ? system_append_dns_mark(system, family, chain, true) : 0;
    }
    if (system_populate_common_output_prefix(
            system, family, chain, local_begin, local_end) != 0) return -1;
    if (!config->matcher.enabled && config->app_policy_mode == STARSEAD_APP_POLICY_BLACKLIST) {
...
static int system_populate_private_chain(
    ...
    enum starsead_ip_family family = group->family;
    const char *local_begin = family == STARSEAD_IP_FAMILY_IPV4
        ? "ASTERISK_LOCAL4_BEGIN" : "ASTERISK_LOCAL6_BEGIN";
    const char *local_end = family == STARSEAD_IP_FAMILY_IPV4
        ? "ASTERISK_LOCAL4_END" : "ASTERISK_LOCAL6_END";
    if (group->chain_id == STARSEAD_CHAIN_LOCAL_BYPASS) {
        return 0;
    }
    if (group->chain_id == STARSEAD_CHAIN_TPROXY) {
    ...
    if (group->chain_id == STARSEAD_CHAIN_ROUTING) {
        if (strstr(chain, "PREROUTING") != NULL) {
        ...
        if (strstr(chain, "FORWARD") != NULL) {
            const struct starsead_config *config = &system->loaded_config.config;
            const char *tunnel = config->helper.value.hev.tunnel_name;
            bool dns_only = family == STARSEAD_IP_FAMILY_IPV6 && !config->enable_ipv6;
            ...
            return system_xtables_zero(system, family, STARSEAD_IP_TABLE_FILTER,
    ...
    if (group->chain_id == STARSEAD_CHAIN_FAKE_DNS) {
        const char *pool = system->loaded_config.config.fake_dns_ipv4_pool;
        const char *arguments[10U];
        size_t argument_count = starsead_xtables_fake_dns_arguments(pool, arguments);
        if (argument_count == 0U) return -1;
        return system_xtables_zero(system, STARSEAD_IP_FAMILY_IPV4,
            STARSEAD_IP_TABLE_NAT, "-A", chain, arguments,
            argument_count);
    }
    return -1;
}

static int system_create_private_group(struct starsead_system_supervisor *system,
    const struct starsead_private_chain_group *group) {
...
static int system_verify_private_chain_contents(
    ...
    if (starsead_xtables_private_chain_shape_valid(
...
static int system_remove_private_group(struct starsead_system_supervisor *system,
    const struct starsead_private_chain_group *group) {
    int result = 0;
    bool snapshot_active =
        system->rule_snapshot.phase == SYSTEM_RULE_SNAPSHOT_BEFORE_APPLY ||
    ...
    for (size_t remaining = group->name_count; remaining > 0U; --remaining) {
        ...
        if (snapshot_active) {
            ...
            size_t rules = 0U;
            if (!view->present || starsead_xtables_private_chain_counts(
...
static int system_apply_hook_group(struct starsead_system_supervisor *system,
    const struct starsead_traffic_hook_group *group) {
    for (size_t index = 0U; index < group->hook_count; ++index) {
        const struct starsead_traffic_hook *hook = &group->hooks[index];
        const char *chain = system_builtin_name(hook->builtin_chain);
        const char *arguments[STARSEAD_XTABLES_MAX_HOOK_ARGUMENTS];
        size_t count = starsead_xtables_hook_arguments(hook, arguments);
        if (chain == NULL || count == 0U) return -1;
        if (hook->insert_at_head) {
            const char *insert[STARSEAD_XTABLES_MAX_HOOK_ARGUMENTS + 1U];
            insert[0] = "1";
...
static int system_hook_match_count(struct starsead_system_supervisor *system,
    const struct starsead_traffic_hook_group *group,
    const struct starsead_traffic_hook *hook, size_t *matches, size_t *position) {
    const char *chain = system_builtin_name(hook->builtin_chain);
    const char *arguments[STARSEAD_XTABLES_MAX_HOOK_ARGUMENTS];
    size_t count = starsead_xtables_hook_arguments(hook, arguments);
    ...
    return starsead_xtables_rule_output_locate(
        observed, observed_length,
        chain, arguments, count, matches, position);
}

static int system_probe_hook_group(struct starsead_system_supervisor *system,
    const struct starsead_traffic_hook_group *group, bool *all_present,
    bool *any_present) {
    *all_present = true;
    *any_present = false;
    for (size_t index = 0U; index < group->hook_count; ++index) {
...
static int system_remove_hook_group(struct starsead_system_supervisor *system,
    const struct starsead_traffic_hook_group *group) {
    int result = 0;
    for (size_t remaining = group->hook_count; remaining > 0U; --remaining) {
        const struct starsead_traffic_hook *hook = &group->hooks[remaining - 1U];
        const char *chain = system_builtin_name(hook->builtin_chain);
        const char *arguments[STARSEAD_XTABLES_MAX_HOOK_ARGUMENTS];
        size_t count = starsead_xtables_hook_arguments(hook, arguments);
        size_t matches = 0U;
...
static const struct starsead_private_chain_group *system_find_private_group(
    const struct starsead_system_supervisor *system,
    const struct starsead_iptables_chain_resource *resource) {
...
static const struct starsead_traffic_hook_group *system_find_hook_group(
    const struct starsead_system_supervisor *system,
    const struct starsead_iptables_rule_resource *resource) {
...
static int system_probe_private_group(struct starsead_system_supervisor *system,
    const struct starsead_private_chain_group *group, bool *all_present,
    bool *any_present) {
    *all_present = true;
    *any_present = false;
    bool snapshot_active =
...
static int system_ip_command(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *const *arguments,
    ...
    size_t count = 0U;
    argv[count++] = "ip";
    argv[count++] = family == STARSEAD_IP_FAMILY_IPV4 ? "-4" : "-6";
    for (size_t index = 0U; index < argument_count; ++index) {
        if (arguments[index] == NULL || arguments[index][0] == '\0') return -1;
        argv[count++] = arguments[index];
    }
...
static int system_ip_zero(struct starsead_system_supervisor *system,
    enum starsead_ip_family family, const char *const *arguments, size_t argument_count) {
    if (system->rule_commands.active) {
        if (family < STARSEAD_IP_FAMILY_IPV4 ||
            family >= STARSEAD_IP_FAMILY_COUNT || argument_count > 20U ||
...
static void system_rule_snapshot_destroy(struct system_rule_snapshot *snapshot) {
    ...
    for (size_t family = 0U; family < STARSEAD_IP_FAMILY_COUNT; ++family) {
        for (size_t table = 0U; table < STARSEAD_IP_TABLE_COUNT; ++table) {
...
static int system_rule_snapshot_capture(
    struct starsead_system_supervisor *system,
    const struct starsead_rule_transaction_plan *plan,
    enum system_rule_snapshot_phase phase) {
    if (system == NULL || plan == NULL ||
        (phase != SYSTEM_RULE_SNAPSHOT_BEFORE_APPLY &&
         phase != SYSTEM_RULE_SNAPSHOT_AFTER_APPLY)) return -1;
    bool tables[STARSEAD_IP_FAMILY_COUNT][STARSEAD_IP_TABLE_COUNT];
    bool rules[STARSEAD_IP_FAMILY_COUNT];
    bool routes[STARSEAD_IP_FAMILY_COUNT];
    memset(tables, 0, sizeof(tables));
    memset(rules, 0, sizeof(rules));
    memset(routes, 0, sizeof(routes));
    for (size_t index = 0U; index < plan->private_group_count; ++index) {
        const struct starsead_private_chain_group *group = &plan->private_groups[index];
        tables[group->family][group->table] = true;
    }
    for (size_t index = 0U; index < plan->hook_group_count; ++index) {
        const struct starsead_traffic_hook_group *group = &plan->hook_groups[index];
        tables[group->family][group->table] = true;
    }
    for (size_t index = 0U; index < plan->route_count; ++index) {
        const struct starsead_route_effect *effect = &plan->routes[index];
        if (effect->kind == STARSEAD_ROUTE_EFFECT_IP_RULE) rules[effect->family] = true;
        if (effect->kind == STARSEAD_ROUTE_EFFECT_ROUTE) routes[effect->family] = true;
    }
    ...
    for (size_t family = 0U; family < STARSEAD_IP_FAMILY_COUNT; ++family) {
        for (size_t table = 0U; table < STARSEAD_IP_TABLE_COUNT; ++table) {
            ...
            int exit_status = -1;
            if (system_xtables_table(system, (enum starsead_ip_family)family,
                    (enum starsead_ip_table)table, &exit_status) != 0 ||
        ...
        if (rules[family]) {
            ...
            int exit_status = -1;
            if (system_ip_command(system, (enum starsead_ip_family)family,
...
static int system_tc_command(struct starsead_system_supervisor *system,
...
static int system_tc_zero(struct starsead_system_supervisor *system,
...
static void system_tc_netlink_close(struct starsead_system_supervisor *system) {
...
static int system_tc_netlink_begin(
    struct starsead_system_supervisor *system, uint32_t interface_index,
    ...
    system->tc_filter_netlink_state = STARSEAD_RULES_SLOT_ABSENT;
    system->tc_qdisc_netlink_state = STARSEAD_RULES_SLOT_ABSENT;
    return 0;
}
...
static int system_tc_netlink_dispatch(
    struct starsead_system_supervisor *system, short ready) {
    ...
    for (;;) {
        ...
        int decoded = system->tc_netlink_query == SYSTEM_TC_NETLINK_QUERY_EXPECTED_FILTER
                ? starsead_tc_filter_netlink_decode(
                ...
                : system->tc_netlink_query == SYSTEM_TC_NETLINK_QUERY_SLOT_FILTER
                    ? starsead_tc_filter_slot_netlink_decode(
                        buffer, (size_t)received, system->tc_netlink_sequence,
                        system->tc_netlink_port_id,
                        &system->tc_filter_netlink_expectation,
                        &system->tc_filter_netlink_state,
                ...
                : system->tc_netlink_query == SYSTEM_TC_NETLINK_QUERY_QDISC
                    ? starsead_tc_qdisc_netlink_decode(
                        buffer, (size_t)received, system->tc_netlink_sequence,
                        system->tc_netlink_port_id,
...
static int system_expected_tc_netlink_probe(
    struct starsead_system_supervisor *system,
    const struct starsead_tc_filter_resource *resource,
    ...
    enum starsead_pin_id pin_id =
        resource->program_id == STARSEAD_PROGRAM_BPF2SOCKS_INGRESS
            ? STARSEAD_PIN_BPF2SOCKS_TC_INGRESS
            : resource->program_id == STARSEAD_PROGRAM_BPF2SOCKS_EGRESS
                ? STARSEAD_PIN_BPF2SOCKS_TC_EGRESS : STARSEAD_PIN_COUNT;
    const struct starsead_b2s_verified_pin *verified =
        system_b2s_verified_pin(system, pin_id);
    const char *name = starsead_b2s_tc_filter_attachment_name(resource->program_id);
    uint32_t parent = resource->direction == STARSEAD_TC_DIRECTION_INGRESS
        ? STARSEAD_TC_PARENT_CLSACT_INGRESS
        : resource->direction == STARSEAD_TC_DIRECTION_EGRESS
            ? STARSEAD_TC_PARENT_CLSACT_EGRESS : 0U;
    ...
    system->tc_filter_netlink_expectation.protocol = STARSEAD_ETH_PROTOCOL_ALL;
    system->tc_filter_netlink_expectation.priority = STARSEAD_HOTSPOT_TC_PRIORITY;
    system->tc_filter_netlink_expectation.handle = STARSEAD_HOTSPOT_TC_HANDLE;
    enum system_tc_netlink_query_kind query = SYSTEM_TC_NETLINK_QUERY_SLOT_FILTER;
    if (verified != NULL && verified->object_id != 0U) {
        query = SYSTEM_TC_NETLINK_QUERY_EXPECTED_FILTER;
        system->tc_filter_netlink_expectation.bpf_flags = STARSEAD_TC_BPF_FLAG_ACT_DIRECT;
    ...
    if (system_tc_filter_netlink_start(system, resource->interface_index,
            parent, STARSEAD_HOTSPOT_TC_PRIORITY, STARSEAD_ETH_PROTOCOL_ALL,
            query) != 0) return -1;
    int64_t now = 0;
    int result = -1;
    if (system_runtime_clock(system, &now) == 0 &&
...
static enum starsead_pin_id system_tc_pin_id(enum starsead_program_id program_id) {
...
static const char *system_tc_direction(enum starsead_tc_direction direction) {
    return direction == STARSEAD_TC_DIRECTION_INGRESS ? "ingress" :
        direction == STARSEAD_TC_DIRECTION_EGRESS ? "egress" : NULL;
}

static int system_tc_probe_qdisc(struct starsead_system_supervisor *system,
    const struct starsead_tc_qdisc_resource *resource, bool *present) {
    if (resource == NULL || present == NULL ||
        if_nametoindex(resource->interface_name) != resource->interface_index) return -1;
    enum starsead_rules_slot_state state = STARSEAD_RULES_SLOT_FOREIGN;
    if (system_qdisc_netlink_probe(system, resource->interface_index, &state) != 0 ||
        state == STARSEAD_RULES_SLOT_FOREIGN) return -1;
    *present = state == STARSEAD_RULES_SLOT_OWNED;
    return 0;
}

static int system_tc_probe_filter(struct starsead_system_supervisor *system,
    const struct starsead_tc_filter_resource *resource,
    enum starsead_rules_slot_state *state) {
...
static int system_tc_apply_qdisc(struct starsead_system_supervisor *system,
    const struct starsead_tc_qdisc_resource *resource) {
    const char *arguments[] = {"qdisc", "add", "dev", resource->interface_name, "clsact"};
    return system_tc_zero(system, arguments, 5U);
}

static int system_tc_apply_filter(struct starsead_system_supervisor *system,
    const struct starsead_tc_filter_resource *resource) {
    const char *direction = system_tc_direction(resource->direction);
    enum starsead_pin_id pin_id = system_tc_pin_id(resource->program_id);
    const char *path = pin_id == STARSEAD_PIN_COUNT ? NULL : system_pin_path(system, pin_id);
...
static int system_apply_route_effect(struct starsead_system_supervisor *system,
    const struct starsead_route_effect *effect) {
    char table[16U];
    char priority[16U];
    char mark[32U];
    ...
    if (effect->kind == STARSEAD_ROUTE_EFFECT_IP_RULE) {
    ...
    if (effect->kind == STARSEAD_ROUTE_EFFECT_ROUTE) {
...
static int system_probe_route_effect(struct starsead_system_supervisor *system,
    const struct starsead_route_effect *effect, bool *present);
static int system_classify_route_effect(struct starsead_system_supervisor *system,
    const struct starsead_route_effect *effect,
    enum starsead_rules_slot_state *state);

static int system_remove_route_effect(struct starsead_system_supervisor *system,
    const struct starsead_route_effect *effect) {
    bool present = false;
...
static int system_classify_route_effect(struct starsead_system_supervisor *system,
    ...
    *state = STARSEAD_RULES_SLOT_ABSENT;
    if (system->rule_snapshot.phase == SYSTEM_RULE_SNAPSHOT_BEFORE_APPLY ||
        system->rule_snapshot.phase == SYSTEM_RULE_SNAPSHOT_AFTER_APPLY) {
        const struct system_rule_view *view =
            effect->kind == STARSEAD_ROUTE_EFFECT_IP_RULE
                ? &system->rule_snapshot.ip_rules[effect->family]
                : &system->rule_snapshot.ip_routes[effect->family];
        if (!view->present) return -1;
        return effect->kind == STARSEAD_ROUTE_EFFECT_IP_RULE
            ? starsead_ip_rule_output_classify(
                view->bytes, view->length, effect, state)
            : starsead_ip_route_output_classify(
                view->bytes, view->length, effect, state);
    }
    int exit_status = -1;
    if (effect->kind == STARSEAD_ROUTE_EFFECT_IP_RULE) {
        ...
        return starsead_ip_rule_output_classify(
    ...
    if (effect->kind == STARSEAD_ROUTE_EFFECT_ROUTE) {
        ...
        if (system_ip_command(system, effect->family, arguments, 5U,
                true, &exit_status) != 0 || exit_status != 0 ||
            system->action_stdout_overflow) return -1;
        return starsead_ip_route_output_classify(
            system->action_stdout, system->action_stdout_length, effect, state);
    }
    return -1;
}

static const struct starsead_route_effect *system_find_route_effect(
    const struct starsead_system_supervisor *system,
    const struct starsead_resource_operation *record) {
    for (size_t index = 0U; index < system->rules_runtime.plan.route_count; ++index) {
        const struct starsead_route_effect *effect = &system->rules_runtime.plan.routes[index];
        if (record->kind == STARSEAD_RESOURCE_OPERATION_IP_RULE &&
            effect->kind == STARSEAD_ROUTE_EFFECT_IP_RULE &&
            effect->family == record->resource.ip_rule.family &&
            effect->ip_rule_id == record->resource.ip_rule.rule_id) return effect;
        if (record->kind == STARSEAD_RESOURCE_OPERATION_ROUTE &&
            effect->kind == STARSEAD_ROUTE_EFFECT_ROUTE &&
...
static int system_route_record(struct starsead_system_supervisor *system,
    const struct starsead_route_effect *effect, struct starsead_resource_operation *record) {
    (void)system;
    memset(record, 0, sizeof(*record));
    if (effect->kind == STARSEAD_ROUTE_EFFECT_IP_RULE) {
        record->kind = STARSEAD_RESOURCE_OPERATION_IP_RULE;
        record->resource.ip_rule.family = effect->family;
        record->resource.ip_rule.rule_id = effect->ip_rule_id;
    } else if (effect->kind == STARSEAD_ROUTE_EFFECT_ROUTE) {
        record->kind = STARSEAD_RESOURCE_OPERATION_ROUTE;
...
static const char *system_pin_path(
    struct starsead_system_supervisor *system, enum starsead_pin_id pin_id) {
    if (!system->matcher_plan_ready && system->loaded_config.config.matcher.enabled &&
        starsead_matcher_pin_plan_build(
    ...
    if (!system->b2s_plan_ready && system->loaded_config.config.mode == STARSEAD_MODE_BPF2SOCKS &&
        starsead_b2s_pin_plan_build(
...
static uint64_t system_verified_pin_id(
    const struct starsead_system_supervisor *system, enum starsead_pin_id pin_id) {
...
static int system_run_matcher_loader(struct starsead_system_supervisor *system) {
    int exit_status = -1;
    char error[256U];
    if (!system->matcher_launch_ready || !system->matcher_plan_ready ||
        ...
        exit_status != 0 || starsead_matcher_verify(
            &system->loaded_config.config, &system->matcher_pin_plan,
            starsead_system_bpf_program_backend(), &system->matcher_verification,
            error, sizeof(error)) != 0) return -1;
...
static bool system_b2s_pins_ready(void *opaque) {
    ...
    char error[128U];
    if (starsead_b2s_verify(
            &system->loaded_config.config, &system->b2s_pin_plan,
            starsead_system_bpf_program_backend(), &system->b2s_verification,
            error, sizeof(error)) == 0) {
...
static int system_start_and_verify_b2s(struct starsead_system_supervisor *system) {
    ...
    return system_pump_condition_periodic(system, &deadline,
            STARSEAD_READINESS_POLL_INTERVAL_MILLIS, system_b2s_pins_ready) == 0 &&
        system->b2s_verified && !system->helper_reaped ? 0 : -1;
}

static int system_sysctl_open(
    const struct starsead_sysctl_resource *resource, int flags, int *output) {
    const char *root_path = resource->sysctl_id == STARSEAD_SYSCTL_DISABLE_IPV6
        ? "/proc/sys/net/ipv6/conf" : "/proc/sys/net/ipv4/conf";
    const char *leaf = resource->sysctl_id == STARSEAD_SYSCTL_DISABLE_IPV6
...
static int system_sysctl_read(
    const struct starsead_sysctl_resource *resource, uint8_t *value) {
    int fd = -1;
...
static int system_effect_probe_original(
    void *opaque, struct starsead_effect *effect,
    char *error, size_t error_size) {
    uint8_t value = 0U;
    (void)opaque;
    if (effect == NULL || effect->kind != STARSEAD_EFFECT_SYSCTL ||
...
static int system_effect_apply_one(
    void *opaque, const struct starsead_effect *effect,
    char *error, size_t error_size) {
    (void)opaque;
    if (effect == NULL || effect->kind != STARSEAD_EFFECT_SYSCTL ||
...
static int system_effect_verify_value(
    const struct starsead_effect *effect, uint8_t expected,
    char *error, size_t error_size) {
    uint8_t value = 0U;
    if (effect == NULL || effect->kind != STARSEAD_EFFECT_SYSCTL ||
        system_sysctl_read(&effect->resource.sysctl, &value) != 0 ||
...
static struct starsead_effect_backend system_effect_backend(
    struct starsead_system_supervisor *system) {
...
static int system_effect_apply_sysctl(
    struct starsead_system_supervisor *system,
    const struct starsead_sysctl_resource *resource,
    char *error, size_t error_size) {
    struct starsead_effect effect;
    struct starsead_effect_backend backend = system_effect_backend(system);
    if (system == NULL || resource == NULL) return -1;
    memset(&effect, 0, sizeof(effect));
    effect.kind = STARSEAD_EFFECT_SYSCTL;
    effect.resource.sysctl = *resource;
    return starsead_effect_journal_apply(
        &system->volatile_effects, &effect, &backend, error, error_size);
}


static bool system_rule_operation_kind(enum starsead_resource_operation_kind kind) {
    return kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_CHAIN ||
        kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_RULE ||
        kind == STARSEAD_RESOURCE_OPERATION_IP_RULE ||
        kind == STARSEAD_RESOURCE_OPERATION_ROUTE;
}

static int system_apply_operations(void *opaque, const struct starsead_resource_operation *records,
    size_t count, char *error, size_t error_size) {
    struct starsead_system_supervisor *system = opaque;
    if (records == NULL || count == 0U) return -1;
    if (count == 1U && records[0].kind == STARSEAD_RESOURCE_OPERATION_SYSCTL) {
        return system_sysctl_write(&records[0].resource.sysctl,
            records[0].resource.sysctl.desired_value);
    }
    if (count == 1U && records[0].kind == STARSEAD_RESOURCE_OPERATION_TC_QDISC) {
        return system_tc_apply_qdisc(system, &records[0].resource.tc_qdisc);
    }
    if (count == 1U && records[0].kind == STARSEAD_RESOURCE_OPERATION_TC_FILTER) {
    ...
    for (size_t index = 0U; rule_batch && index < count; ++index) {
        enum starsead_resource_operation_kind kind = records[index].kind;
        rule_batch = kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_CHAIN ||
            kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_RULE ||
            kind == STARSEAD_RESOURCE_OPERATION_IP_RULE ||
            kind == STARSEAD_RESOURCE_OPERATION_ROUTE;
    }
    if (rule_batch) {
        ...
        for (size_t index = 0U; index < count; ++index) {
            const struct starsead_resource_operation *record = &records[index];
            int result = -1;
            if (record->kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_CHAIN) {
                const struct starsead_private_chain_group *group =
            ...
            } else if (record->kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_RULE) {
                const struct starsead_traffic_hook_group *group =
                    system_find_hook_group(system, &record->resource.iptables_rule);
                result = group == NULL ? -1 : system_apply_hook_group(system, group);
            } else if (record->kind == STARSEAD_RESOURCE_OPERATION_IP_RULE ||
                record->kind == STARSEAD_RESOURCE_OPERATION_ROUTE) {
                const struct starsead_route_effect *effect =
...
rule_batch_failed:
    ...
    if (records[0].kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_CHAIN) {
        const struct starsead_private_chain_group *group =
    ...
    if (count == 1U && records[0].kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_RULE) {
        const struct starsead_traffic_hook_group *group =
    ...
    if (count == 1U && (records[0].kind == STARSEAD_RESOURCE_OPERATION_IP_RULE ||
            records[0].kind == STARSEAD_RESOURCE_OPERATION_ROUTE)) {
        const struct starsead_route_effect *effect = system_find_route_effect(system, &records[0]);
        return effect == NULL ? -1 : system_apply_route_effect(system, effect);
    }
    for (size_t index = 0U; index < count; ++index) {
        if (records[index].kind != STARSEAD_RESOURCE_OPERATION_BPF_PIN) return -1;
    }
    if (!system->pin_batch_active) return -1;
    bool matcher_batch = system->active_pin_batch == STARSEAD_PIN_BATCH_MATCHER_IPV4 ||
        system->active_pin_batch == STARSEAD_PIN_BATCH_MATCHER_DUAL_STACK;
    bool b2s_batch = system->active_pin_batch == STARSEAD_PIN_BATCH_BPF2SOCKS_IPV4 ||
        system->active_pin_batch == STARSEAD_PIN_BATCH_BPF2SOCKS_DUAL_STACK;
...
static int system_verify_operation(void *opaque,
    ...
    if (record->kind == STARSEAD_RESOURCE_OPERATION_IPTABLES_RULE) {
        const struct starsead_traffic_hook_group *group =
            system_find_hook_group(system, &record->resource.iptables_rule);
        bool all_present = false;
        bool any_present = false;
    ...
    if (record->kind == STARSEAD_RESOURCE_OPERATION_IP_RULE ||
        record->kind == STARSEAD_RESOURCE_OPERATION_ROUTE) {
        const struct starsead_route_effect *effect = system_find_route_effect(system, record);
        bool present = false;
...
static int system_apply_and_verify(
    struct starsead_system_supervisor *system,
    const struct starsead_resource_operation *records,
    size_t count, char *error, size_t error_size) {
    size_t index;
...
static int system_rules_apply_plan(
    void *opaque, const struct starsead_rule_transaction_plan *plan) {
    struct starsead_system_supervisor *system = opaque;
    if (plan == NULL || plan->no_op) return plan != NULL ? 0 : -1;
    struct starsead_resource_operation records[
        STARSEAD_RULE_TRANSACTION_MAX_GROUPS * 2U +
        STARSEAD_RULE_TRANSACTION_MAX_ROUTES];
    size_t count = 0U;
    for (size_t index = 0U; index < plan->private_group_count; ++index) {
...
static int system_rules_apply_private(
    void *opaque, const struct starsead_private_chain_group *group) {
    struct starsead_system_supervisor *system = opaque;
    struct starsead_resource_operation record = group->operation;
    char error[128U] = {0};
    int applied = system_apply_and_verify(
...
static int system_rules_apply_route(
    void *opaque, const struct starsead_route_effect *effect) {
    struct starsead_system_supervisor *system = opaque;
...
static int system_rules_apply_hook(
    void *opaque, const struct starsead_traffic_hook_group *group) {
    struct starsead_system_supervisor *system = opaque;
    struct starsead_resource_operation record = group->operation;
    char error[128U] = {0};
    int applied = system_apply_and_verify(
...
static int system_rules_probe_private(void *opaque,
    ...
    if (!any_present) *state = STARSEAD_RULES_SLOT_ABSENT;
    else if (all_present) {
        *state = system_verify_private_group_contents(system, group) == 0
            ? STARSEAD_RULES_SLOT_OWNED : STARSEAD_RULES_SLOT_FOREIGN;
    } else {
...
static void system_rules_backend_init(struct starsead_system_supervisor *system) {
    starsead_rules_runtime_init(&system->rules_runtime);
    system->rules_backend = (struct starsead_rules_backend){
        .ctx = system,
...
static int system_effect_start_matcher(void *opaque) {
    struct starsead_system_supervisor *system = opaque;
    const struct starsead_bpf_pin_ownership_backend *pin_backend =
        starsead_system_bpf_pin_ownership_backend();
    char error[256U];
    if (system->matcher_launch_ready || system->matcher_verified ||
        starsead_matcher_pin_plan_build(
            &system->loaded_config.config, &system->matcher_pin_plan) != 0) return -1;
    system->matcher_plan_ready = true;
    if (pin_backend == NULL || starsead_matcher_pin_preflight(
            &system->matcher_pin_plan, pin_backend, error, sizeof(error)) != 0 ||
        starsead_matcher_launch_prepare(
            &system->loaded_config.config, (const char *const *)environ,
            starsead_system_anonymous_file_backend(), &system->matcher_launch,
            error, sizeof(error)) != 0) return -1;
    system->matcher_launch_ready = true;
    struct starsead_resource_operation records[4U];
    size_t count = 0U;
    if (starsead_matcher_pin_records_build(
    ...
    int destroyed = starsead_matcher_launch_destroy(
        starsead_system_anonymous_file_backend(), &system->matcher_launch);
...
static int system_effect_start_helper(
    ...
    if (system->loaded_config.config.helper.type != STARSEAD_HELPER_BPF2SOCKS) {
        return -1;
    }
    const struct starsead_bpf_pin_ownership_backend *pin_backend =
        starsead_system_bpf_pin_ownership_backend();
    char error[256U];
    if (starsead_b2s_pin_plan_build(
            &system->loaded_config.config, &system->b2s_pin_plan) != 0) return -1;
    system->b2s_plan_ready = true;
    if (pin_backend == NULL || starsead_b2s_pin_preflight(
            &system->b2s_pin_plan, pin_backend, error, sizeof(error)) != 0) return -1;
    struct starsead_resource_operation records[4U];
    size_t count = 0U;
    if (starsead_b2s_pin_records_build(
...
static int system_effect_open_network(void *opaque) {
    ...
    if (starsead_network_set_effect_sink(
...
static int system_collect_local_address_set(
    struct starsead_system_supervisor *system,
    int family,
    struct starsead_address_set *set) {
    struct ifaddrs *addresses = NULL;
    if (getifaddrs(&addresses) != 0) return -1;
    struct starsead_interface_address candidates[STARSEAD_MAX_ADDRESSES];
    size_t count = 0U;
    ...
    for (const struct ifaddrs *entry = addresses; entry != NULL; entry = entry->ifa_next) {
        ...
        if (count >= STARSEAD_MAX_ADDRESSES) {
    ...
    freeifaddrs(addresses);
    char error[128U];
    return starsead_local_address_set_build(&system->loaded_config.config,
        family, candidates, count, set, error, sizeof(error)) == 0 ? 0 : -1;
}

static bool system_uses_iptables_local_bypass(
    const struct starsead_system_supervisor *system) {
    enum starsead_mode mode = system->loaded_config.config.mode;
    return mode == STARSEAD_MODE_TPROXY ||
        mode == STARSEAD_MODE_TUN2SOCKS;
}
...
static int system_capture_local_address_snapshot(
    ...
    if (system_collect_local_address_set(system, STARSEAD_ADDRESS_IPV4,
            &system->local_ipv4_snapshot) != 0) return -1;
    if (system->loaded_config.config.enable_ipv6) {
        if (system_collect_local_address_set(system, STARSEAD_ADDRESS_IPV6,
                &system->local_ipv6_snapshot) != 0) return -1;
    } else {
        system->local_ipv6_snapshot.family = STARSEAD_ADDRESS_IPV6;
    }
...
static int system_apply_local_bypass_plan(
    ...
    enum starsead_ip_family family,
    const char *consumer_chain,
    const struct starsead_local_bypass_plan *plan) {
    for (size_t index = 0U; index < plan->operation_count; ++index) {
        const struct starsead_local_bypass_operation *operation = &plan->operations[index];
        char cidr[STARSEAD_MAX_CIDR];
        int written = snprintf(cidr, sizeof(cidr), "%s/%u", operation->address,
            family == STARSEAD_IP_FAMILY_IPV4 ? 32U : 128U);
        if (written <= 0 || (size_t)written >= sizeof(cidr)) return -1;
        if (operation->kind == STARSEAD_LOCAL_BYPASS_INSERT) {
            char position[24U];
        ...
        } else if (operation->kind == STARSEAD_LOCAL_BYPASS_DELETE) {
            const char *arguments[] = {"-d", cidr, "-j", "RETURN"};
            if (system_xtables_zero(system, family, STARSEAD_IP_TABLE_MANGLE,
...
static int system_reconcile_local_bypass_consumer(
    ...
    int exit_status = -1;
    if (system_xtables_run(system, family, STARSEAD_IP_TABLE_MANGLE,
    ...
    int address_family = family == STARSEAD_IP_FAMILY_IPV4
        ? STARSEAD_ADDRESS_IPV4 : STARSEAD_ADDRESS_IPV6;
    if (starsead_local_bypass_plan_build(address_family, consumer_chain,
...
static int system_reconcile_iptables_local_family(
    struct starsead_system_supervisor *system,
    enum starsead_ip_family family) {
    const struct starsead_private_chain_group *local_group = NULL;
    const struct starsead_private_chain_group *consumer_group = NULL;
    enum starsead_chain_id consumer_id = system->loaded_config.config.mode ==
        STARSEAD_MODE_TPROXY ? STARSEAD_CHAIN_TPROXY : STARSEAD_CHAIN_ROUTING;
    for (size_t index = 0U; index < system->rules_runtime.plan.private_group_count; ++index) {
        const struct starsead_private_chain_group *group =
            &system->rules_runtime.plan.private_groups[index];
        if (group->family != family || group->table != STARSEAD_IP_TABLE_MANGLE) continue;
        if (group->chain_id == STARSEAD_CHAIN_LOCAL_BYPASS) local_group = group;
    ...
    int address_family = family == STARSEAD_IP_FAMILY_IPV4
        ? STARSEAD_ADDRESS_IPV4 : STARSEAD_ADDRESS_IPV6;
    const struct starsead_address_set *desired = family == STARSEAD_IP_FAMILY_IPV4
        ? &system->local_ipv4_snapshot : &system->local_ipv6_snapshot;
...
static int system_reconcile_iptables_local_bypass(
    ...
    if (system_reconcile_iptables_local_family(
            system, STARSEAD_IP_FAMILY_IPV4) != 0) return -1;
    return !system->loaded_config.config.enable_ipv6 ||
        system_reconcile_iptables_local_family(
            system, STARSEAD_IP_FAMILY_IPV6) == 0 ? 0 : -1;
}

static int system_reconcile_b2s_local_maps(
    struct starsead_system_supervisor *system) {
    if (system->loaded_config.config.mode != STARSEAD_MODE_BPF2SOCKS) return 0;
    const struct starsead_bpf_map_backend *backend = starsead_system_bpf_map_backend();
    if (!system->b2s_verified || backend == NULL) return -1;
    struct starsead_address_set addresses;
    char error[128U];
    const char *path = system_pin_path(
        system, STARSEAD_PIN_BPF2SOCKS_LOCAL_ADDRESS_V4);
    uint64_t object_id = system_verified_pin_id(
        system, STARSEAD_PIN_BPF2SOCKS_LOCAL_ADDRESS_V4);
    if (path == NULL || object_id == 0U ||
        system_collect_local_address_set(
            system, STARSEAD_ADDRESS_IPV4, &addresses) != 0 ||
        starsead_bpf_local_map_reconcile(backend, path, object_id,
            &addresses, error, sizeof(error)) != 0) return -1;
    if (!system->loaded_config.config.enable_ipv6) return 0;
    path = system_pin_path(system, STARSEAD_PIN_BPF2SOCKS_LOCAL_ADDRESS_V6);
    object_id = system_verified_pin_id(
        system, STARSEAD_PIN_BPF2SOCKS_LOCAL_ADDRESS_V6);
    return path != NULL && object_id != 0U &&
        system_collect_local_address_set(system, STARSEAD_ADDRESS_IPV6, &addresses) == 0 &&
        starsead_bpf_local_map_reconcile(backend, path, object_id,
            &addresses, error, sizeof(error)) == 0 ? 0 : -1;
}

static bool system_hotspot_interface_selected(
    const struct starsead_config *config, const char *name) {
    for (size_t index = 0U; index < config->hotspot_interface_prefix_count; ++index) {
...
static int system_clear_android_hotspot_offload_interface(
    struct starsead_system_supervisor *system, const char *name, const char *priority) {
    const char *show[] = {
    ...
    if (system_tc_command(system, show, 9U, true, &exit_status) != 0 ||
        exit_status != 0 || system->action_stdout_overflow) return -1;
    if (!starsead_hotspot_tc_output_has_android_offload(
...
static int system_clear_android_hotspot_offload(
    struct starsead_system_supervisor *system,
    enum starsead_hotspot_ipv6_offload_policy policy) {
    bool required = policy == STARSEAD_HOTSPOT_IPV6_OFFLOAD_REMOVE_PREF1_REQUIRED;
...
static int system_tc_slot_for_qdisc(struct starsead_system_supervisor *system,
    const struct starsead_resource_operation *prototype,
    enum starsead_tc_slot_state *slot) {
    bool present = false;
    if (system_tc_probe_qdisc(system, &prototype->resource.tc_qdisc, &present) != 0) return -1;
    if (!present) *slot = STARSEAD_TC_SLOT_ABSENT;
    else *slot = STARSEAD_TC_SLOT_COMPATIBLE;
    return 0;
}

static int system_tc_slot_for_filter(struct starsead_system_supervisor *system,
    const struct starsead_resource_operation *prototype,
    enum starsead_tc_slot_state *slot) {
    enum starsead_rules_slot_state state = STARSEAD_RULES_SLOT_FOREIGN;
    if (system_tc_probe_filter(system, &prototype->resource.tc_filter, &state) != 0) return -1;
    if (state == STARSEAD_RULES_SLOT_ABSENT) *slot = STARSEAD_TC_SLOT_ABSENT;
    else if (state == STARSEAD_RULES_SLOT_OWNED) {
        *slot = STARSEAD_TC_SLOT_OWNED;
    } else {
        *slot = STARSEAD_TC_SLOT_FOREIGN;
    }
    return 0;
}

static int system_tc_apply_record(struct starsead_system_supervisor *system,
    const struct starsead_resource_operation *prototype) {
    char error[128U];
    if (prototype->kind == STARSEAD_RESOURCE_OPERATION_SYSCTL) {
        return system_effect_apply_sysctl(
            system, &prototype->resource.sysctl, error, sizeof(error));
    }
    struct starsead_resource_operation record = *prototype;
    return system_apply_and_verify(
        system, &record, 1U, error, sizeof(error));
}
...
static int system_reconcile_hotspot_tc_interface(
    struct starsead_system_supervisor *system, const char *name, uint32_t index) {
    struct starsead_tc_interface_probe probe;
    ...
    sysctl.sysctl_id = STARSEAD_SYSCTL_ROUTE_LOCALNET;
    ...
    struct starsead_resource_operation qdisc;
    memset(&qdisc, 0, sizeof(qdisc));
    qdisc.kind = STARSEAD_RESOURCE_OPERATION_TC_QDISC;
    qdisc.resource.tc_qdisc.qdisc_id = STARSEAD_QDISC_HOTSPOT_CLSACT;
    ...
    struct starsead_resource_operation filter;
    memset(&filter, 0, sizeof(filter));
    filter.kind = STARSEAD_RESOURCE_OPERATION_TC_FILTER;
    ...
    filter.resource.tc_filter.filter_id = STARSEAD_FILTER_HOTSPOT_EGRESS;
    filter.resource.tc_filter.direction = STARSEAD_TC_DIRECTION_EGRESS;
    filter.resource.tc_filter.program_id = STARSEAD_PROGRAM_BPF2SOCKS_EGRESS;
    if (system_tc_slot_for_filter(system, &filter, &probe.egress) != 0) return -1;
    filter.resource.tc_filter.filter_id = STARSEAD_FILTER_HOTSPOT_INGRESS;
    filter.resource.tc_filter.direction = STARSEAD_TC_DIRECTION_INGRESS;
    filter.resource.tc_filter.program_id = STARSEAD_PROGRAM_BPF2SOCKS_INGRESS;
    if (system_tc_slot_for_filter(system, &filter, &probe.ingress) != 0) return -1;

    struct starsead_tc_plan plan;
    char error[128U];
    if (starsead_tc_install_plan_build(&system->loaded_config.config,
...
static int system_reconcile_hotspot_tc(struct starsead_system_supervisor *system) {
    bool bpf2socks = system->loaded_config.config.mode == STARSEAD_MODE_BPF2SOCKS;
    enum starsead_hotspot_ipv6_offload_policy offload_policy =
        starsead_hotspot_ipv6_offload_policy_for(&system->loaded_config.config);
    if (offload_policy != STARSEAD_HOTSPOT_IPV6_OFFLOAD_KEEP &&
        system_clear_android_hotspot_offload(system, offload_policy) != 0) return -1;
    if (!bpf2socks) return 0;
    struct ifaddrs *addresses = NULL;
    if (getifaddrs(&addresses) != 0) return -1;
    char handled[STARSEAD_MAX_ADDRESSES][STARSEAD_MAX_INTERFACE_NAME];
    ...
    for (const struct ifaddrs *entry = addresses; entry != NULL; entry = entry->ifa_next) {
        ...
        if (handled_count >= STARSEAD_MAX_ADDRESSES) {
            result = -1;
            break;
        }
...
static uint32_t system_rule_categories(const struct starsead_config *config) {
    uint32_t categories = 0U;
    if (config->mode == STARSEAD_MODE_TPROXY) {
        categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_TPROXY) |
            STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_ROUTING) |
            STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_LOCAL_BYPASS);
    } else if (config->mode == STARSEAD_MODE_TUN2SOCKS) {
        categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_ROUTING) |
            STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_LOCAL_BYPASS);
    } else if (config->mode == STARSEAD_MODE_BPF2SOCKS) {
        categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_BPF);
        if (config->hotspot_interface_prefix_count != 0U) {
            categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_TC) |
                STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_HOTSPOT);
        }
    }
    if (config->matcher.enabled) {
        categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_BPF);
    ...
    if (config->hotspot_interface_prefix_count != 0U) {
        categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_HOTSPOT);
    }
    if (config->enable_local_dns) {
        categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_DNS);
    }
    if (config->enable_fake_dns) {
        categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_FAKE_DNS);
    }
    if (config->disable_system_ipv6) {
        categories |= STARSEAD_RULE_CATEGORY_BIT(STARSEAD_RULE_CATEGORY_IPV6_GUARD);
    }
    return categories;
}
...
static size_t system_owned_hook_arguments(
    const struct starsead_owned_hook *hook,
...
static const struct starsead_owned_chain *system_owned_hook_target_chain(
    const struct starsead_owned_hook *hook) {
    ...
    for (size_t index = 0U; index < catalog->chain_count; ++index) {
        const struct starsead_owned_chain *candidate = &catalog->chains[index];
...
static bool system_owned_hook_rule_probe_required(
    const struct starsead_owned_hook *hook, bool target_chain_present) {
...
static int system_owned_chain_presence(
    struct starsead_system_supervisor *,
    const struct starsead_owned_chain *, bool *);
...
static int system_reconcile_hook(
    ...
    if (argument_count == 0U) return -1;
    const struct starsead_owned_chain *target_chain =
...
static int system_reconcile_hooks(
    struct starsead_system_supervisor *system, bool remove) {
    const struct starsead_owned_resource_catalog *catalog =
        starsead_owned_resource_catalog();
    size_t index;
    int result = 0;
...
static int system_owned_chain_presence(
    struct starsead_system_supervisor *system,
    const struct starsead_owned_chain *chain,
    bool *present) {
    int exit_status = -1;
...
static int system_reconcile_private_chains(
    ...
    for (index = 0U; index < catalog->chain_count; ++index) {
        const struct starsead_owned_chain *chain = &catalog->chains[index];
        bool present = false;
        int exit_status = -1;
    ...
    for (index = catalog->chain_count; index > 0U; --index) {
        const struct starsead_owned_chain *chain = &catalog->chains[index - 1U];
        bool present = false;
        int exit_status = -1;
...
static int system_owned_policy_rule_count(
    ...
    if (count == NULL ||
        ...
        exit_status != 0 || system->action_stdout_overflow) return -1;
    return starsead_owned_policy_rule_output_count(
...
static void system_owned_token_effect(struct starsead_route_effect *effect) {
    const struct starsead_owned_token_route *token =
        &starsead_owned_resource_catalog()->token_route;
    ...
    effect->route_id = STARSEAD_ROUTE_TOKEN;
...
static int system_reconcile_token_route(
    struct starsead_system_supervisor *system, bool remove) {
    struct starsead_route_effect effect;
    enum starsead_rules_slot_state state = STARSEAD_RULES_SLOT_ABSENT;
    size_t attempt;
    ...
    for (attempt = 0U; attempt < 64U; ++attempt) {
        if (system_classify_route_effect(system, &effect, &state) != 0 ||
            state == STARSEAD_RULES_SLOT_FOREIGN) return -1;
        if (state == STARSEAD_RULES_SLOT_ABSENT) return 0;
...
static int system_prepare_owned_tc_identity(
    ...
    if (starsead_b2s_pin_plan_build(
        ...
        starsead_b2s_verify_residue(
            &config, &system->b2s_pin_plan,
            starsead_system_bpf_program_backend(),
            starsead_system_bpf_pin_ownership_backend(),
...
static int system_reconcile_tc_filter(
    ...
    bool remove) {
    struct starsead_tc_filter_resource filter;
    enum starsead_rules_slot_state state = STARSEAD_RULES_SLOT_FOREIGN;
    ...
    filter.filter_id = direction == STARSEAD_TC_DIRECTION_INGRESS
    ...
    if (state == STARSEAD_RULES_SLOT_ABSENT ||
        state == STARSEAD_RULES_SLOT_FOREIGN) return 0;
    if (!remove) return -1;
    ...
    if (system_tc_command(system, arguments,
        ...
        state != STARSEAD_RULES_SLOT_ABSENT) return -1;
    return 0;
}
...
static int system_reconcile_tc_filters(
    ...
    while ((entry = readdir(directory)) != NULL) {
        ...
        size_t length = strnlen(
            entry->d_name, STARSEAD_MAX_INTERFACE_NAME);
        ...
        if (system_reconcile_tc_filter(system, entry->d_name, interface_index,
                ...
                STARSEAD_PROGRAM_BPF2SOCKS_INGRESS, remove) != 0) result = -1;
        if (system_reconcile_tc_filter(system, entry->d_name, interface_index,
                STARSEAD_TC_DIRECTION_EGRESS,
                STARSEAD_PROGRAM_BPF2SOCKS_EGRESS, remove) != 0) {
            result = -1;
        }
    }
    if (closedir(directory) != 0) result = -1;
...
static int system_reconcile_remove_phase(
    ...
    switch (phase) {
        case STARSEAD_RECONCILE_QUIESCE: {
    ...
    if (result != 0) {
        ...
        int written = snprintf(message, sizeof(message),
            ...
            phase >= STARSEAD_RECONCILE_QUIESCE &&
...
static void system_reconcile_warn(void *opaque,
    enum starsead_reconcile_phase phase, const char *detail) {
    struct starsead_system_supervisor *system = opaque;
    static const char *const names[] = {
    ...
    (void)snprintf(message, sizeof(message),
        ...
        phase >= STARSEAD_RECONCILE_QUIESCE && phase <= STARSEAD_RECONCILE_PHASE_COUNT
            ? names[phase] : "invalid", detail);
    (void)starsead_log_line(&system->logger, STARSEAD_LOG_LEVEL_WARNING,
        STARSEAD_COMPONENT_RULES, STARSEAD_LOG_EVENT_DIAGNOSTIC, message);
}

static struct starsead_reconcile_backend system_reconcile_backend(
    struct starsead_system_supervisor *system) {
...
static int system_reconcile_owned_resources_after_listener(
    ...
    char *error,
    size_t error_size) {
    struct starsead_reconcile_backend backend =
    ...
    int result = starsead_reconcile_after_listener(
...
static int system_reconcile_owned_resources(
    ...
    return system_reconcile_owned_resources_after_listener(
        system, STARSEAD_CONTROL_LISTENER_OK, error, error_size);
}
...
static int system_effect_rules(void *opaque, bool *active,
    ...
    if (system->network_opened && !system->network.no_op) {
        ...
        if (system_runtime_clock(system, &now) != 0 ||
            starsead_network_handle(
                &system->network, (uint64_t)now, error, sizeof(error)) != 0 ||
            starsead_network_apply_immediate(
    ...
    } else if (starsead_rules_install(&system->rules_runtime,
    ...
    if (starsead_mode_core_managed(system->loaded_config.config.mode)) {
        *active = false;
        *generation = 0U;
        *categories = 0U;
    } else {
...
static int system_effect_reconcile(void *opaque, bool *active,
    ...
    } else if (starsead_rules_reconcile(
            &system->rules_runtime, &system->rules_backend) != 0) {
...
static int system_effect_network_immediate(void *opaque) {
    struct starsead_system_supervisor *system = opaque;
    char error[128U];
    return system->network_opened && starsead_network_apply_immediate(
...
static int system_effect_verify(void *opaque) {
    ...
    int result = runtime_startup_components_verified(
        ...
        system->loaded_config.config.helper.type != STARSEAD_HELPER_NONE,
...
static bool system_stop_done(void *opaque) {
    ...
    system->stop_result = starsead_stop_coordinator_poll(
        &system->stop_coordinator, &system->stop_backend, (uint64_t)now);
    return system->stop_result != STARSEAD_STOP_PENDING;
}
...
static int system_effect_stop_children(
    ...
    if (system->stop_result == STARSEAD_STOP_COMPLETE) {
...
static int system_effect_stop_helper(void *opaque) {
    ...
    int pins_cleaned = system->loaded_config.config.helper.type == STARSEAD_HELPER_BPF2SOCKS
        ? system_reconcile_remove_phase(
            system, STARSEAD_RECONCILE_BPF_PINS, error, sizeof(error))
        : 0;
...
static int system_effect_quiesce(void *opaque) {
    ...
    struct starsead_rule_transaction_plan snapshot_plan =
...
static int system_effect_remove_rules(void *opaque) {
    struct starsead_system_supervisor *system = opaque;
    system->cleanup_in_progress = true;
    if (starsead_mode_core_managed(system->loaded_config.config.mode)) return 0;
    if (!system->rules_initialized || starsead_rules_remove(
            &system->rules_runtime, &system->rules_backend) != 0) return -1;
    if (starsead_runtime_remove_tc_before_helper_stop(
            system->loaded_config.config.mode)) {
        char error[128U];
        if (system_reconcile_remove_phase(system,
                STARSEAD_RECONCILE_QUIESCE, error, sizeof(error)) != 0) {
            return -1;
        }
    }
    return 0;
...
static int system_effect_close_network(void *opaque) {
    ...
    int result = starsead_network_close(&system->network);
...
static int system_effect_restore_best_effort(void *opaque) {
    ...
    char error[128U];
    struct starsead_effect_backend backend = system_effect_backend(system);
    return starsead_effect_journal_rollback(
        &system->volatile_effects, &backend, error, sizeof(error));
}

static struct starsead_runtime_effect_backend system_runtime_effects(
    struct starsead_system_supervisor *system) {
    const struct starsead_runtime_effect_backend effects = {
        .context = system,
...
static int system_start_result(struct starsead_control_result *result,
    enum starsead_control_result_code code, const char *message,
    const struct starsead_control_snapshot *snapshot) {
    starsead_control_result_destroy(result);
    result->code = code;
    if (snapshot != NULL) {
        result->has_snapshot = true;
        if (starsead_control_snapshot_copy(&result->snapshot, snapshot) != 0) return -1;
    }
    return starsead_control_result_set_message(result, message, strlen(message));
}

static int system_probe_existing(struct starsead_control_result *result) {
    struct starsead_control_response response;
    memset(&response, 0, sizeof(response));
    enum starsead_control_client_result probed = starsead_control_client_run(
        STARSEAD_CONTROL_METHOD_STATUS, "start", NULL, NULL, &response);
    int status;
    if (probed == STARSEAD_CONTROL_CLIENT_OK && response.result.has_snapshot) {
        status = system_start_result(result, STARSEAD_CONTROL_RESULT_ALREADY_RUNNING,
            "protocol-valid supervisor owns the runtime", &response.result.snapshot);
    } else {
        status = system_start_result(result, STARSEAD_CONTROL_RESULT_START_FAILED,
...
static bool system_control_drained(void *opaque) {
    struct starsead_system_supervisor *system = opaque;
    return starsead_control_server_drained(system->control);
}

static void system_child_process_defaults(struct starsead_child_process *process) {
...
static void system_runtime_cleanup_cycle(struct starsead_system_supervisor *system) {
    system_rule_batch_destroy(&system->rule_commands);
    system_rule_snapshot_destroy(&system->rule_snapshot);
    system_tc_netlink_close(system);
    if (system->network_opened) (void)starsead_network_close(&system->network);
    system->network_opened = false;
    starsead_child_process_close(&system->core_process);
    starsead_child_process_close(&system->helper_process);
    starsead_child_process_close(&system->action_process);
    if (system->core_spec_ready) starsead_process_spec_destroy(&system->core_spec);
    system->core_spec_ready = false;
    if (system->helper_launch_ready) {
        (void)starsead_helper_launch_destroy(
            starsead_system_anonymous_file_backend(), &system->helper_launch);
    }
    system->helper_launch_ready = false;
    (void)system_tun_compatibility_cleanup(system);
    if (system->matcher_launch_ready) {
        (void)starsead_matcher_launch_destroy(
            starsead_system_anonymous_file_backend(), &system->matcher_launch);
    }
    system->matcher_launch_ready = false;
}

static void system_runtime_reset_cycle(struct starsead_system_supervisor *system) {
    system_runtime_cleanup_cycle(system);
    size_t offset = offsetof(struct starsead_system_supervisor, stop_requested);
    memset((unsigned char *)system + offset, 0, sizeof(*system) - offset);
    starsead_tun_compat_state_init(&system->tun_compatibility);
    system->tc_netlink_fd = -1;
...
static int system_reset_telemetry_state(
    struct starsead_system_supervisor *system,
    char *error, size_t error_size) {
    struct starsead_state_document replacement;
    int result = starsead_state_document_init(&replacement,
        system->loaded_config.config.owner,
        system->loaded_config.config.core_type,
        system->loaded_config.config.mode);
    if (result != STARSEAD_STATE_OK) return result;
    result = starsead_state_store_save(
        &system->store, &replacement, error, error_size);
    if (result != STARSEAD_STATE_OK) {
        starsead_state_document_destroy(&replacement);
        return result;
    }
    if (system->state.initialized) {
        starsead_state_document_destroy(&system->state);
    }
    system->state = replacement;
    return STARSEAD_STATE_OK;
}

static void system_runtime_cleanup(struct starsead_system_supervisor *system) {
    system_runtime_cleanup_cycle(system);
    if (system->control != NULL) {
        starsead_control_server_destroy(system->control);
        system->control = NULL;
    }
    ...
    system->runtime = NULL;
    starsead_state_document_destroy(&system->state);
    starsead_state_store_close(&system->store);
    if (system->logger.opened) (void)starsead_log_close(&system->logger);
    starsead_loaded_config_release(&system->loaded_config);
}
...
static void system_runtime_finish_control_stop(
    ...
    if (starsead_control_snapshot_from_state(&system->state, &system->live, &snapshot) == 0) {
        result.code = starsead_state_is_stopped(&system->state)
            ? STARSEAD_CONTROL_RESULT_OK : STARSEAD_CONTROL_RESULT_STOP_FAILED;
        result.has_snapshot = true;
        (void)starsead_control_snapshot_copy(&result.snapshot, &snapshot);
        if (result.code == STARSEAD_CONTROL_RESULT_STOP_FAILED) {
            (void)starsead_control_result_set_message(&result, "cleanup incomplete", 18U);
        }
        int64_t now = 0;
        if (system_runtime_clock(system, &now) == 0) {
            (void)starsead_control_server_finish_stop(
...
static int system_runtime_run(const char *config_path, bool initial_start,
    bool *has_early_result, struct starsead_control_result *early_result) {
    ...
    *has_early_result = false;
    ...
    system_child_process_defaults(&system.action_process);
    char error[256U];
    int loaded_config = starsead_config_load(
        config_path, &system.loaded_config, error, sizeof(error));
    if (loaded_config != 0) {
        *has_early_result = true;
        enum starsead_control_result_code code =
            loaded_config == STARSEAD_CONFIG_UNSUPPORTED_COMBINATION
    ...
    int listener = -1;
    enum starsead_control_listener_result listened = system.signal_fd_owned
        ? starsead_control_listener_open(&listener) : STARSEAD_CONTROL_LISTENER_ERROR;
    if (listened != STARSEAD_CONTROL_LISTENER_OK) {
        *has_early_result = true;
        if (listened == STARSEAD_CONTROL_LISTENER_IN_USE) {
        ...
        system_runtime_cleanup(&system);
        (void)sigprocmask(SIG_SETMASK, &previous, NULL);
        return starsead_control_result_exit_code(early_result->code);
    }
    ...
    if (state_result != STARSEAD_STATE_OK) {
        (void)close(listener);
        *has_early_result = true;
        (void)system_start_result(early_result, STARSEAD_CONTROL_RESULT_START_FAILED,
    ...
    starsead_service_control_init(&system.service_control,
        &system.loaded_config.config.service_control, initial_start, service_now);
    const struct starsead_service_control_config *service =
        &system.loaded_config.config.service_control;
    ...
    if (system.wifi_monitor_opened) {
        ...
        struct starsead_wifi_identity identity;
        if (starsead_wifi_monitor_baseline(
    ...
    if (starsead_control_server_create(&system.control, listener, &callbacks) != 0) {
        (void)close(listener);
        *has_early_result = true;
        (void)system_start_result(early_result, STARSEAD_CONTROL_RESULT_START_FAILED,
    ...
    struct starsead_runtime_reactor_backend reactor_backend = system_runtime_reactor_backend();
    if (starsead_runtime_create(&system.runtime, &reactor_backend, &system) != 0) {
        *has_early_result = true;
        (void)system_start_result(early_result, STARSEAD_CONTROL_RESULT_START_FAILED,
    ...
    if (!admission_reconcile_ready(
            listened == STARSEAD_CONTROL_LISTENER_OK, system.runtime != NULL) ||
    ...
    starsead_control_server_enable_accepting(system.control, true);
    ...
    while (!system.shutdown_requested) {
        if (should_start || system.service_start_requested) {
            if (!should_start) {
                if (system_reset_telemetry_state(
                        &system, error, sizeof(error)) != STARSEAD_STATE_OK) {
                    status = 1;
                    system.shutdown_requested = true;
                    break;
                }
            ...
            starsead_service_control_set_service_running(&system.service_control, true);
            int cycle_status = starsead_runtime_supervise(
        ...
        struct starsead_runtime_delta delta;
        if (starsead_runtime_pump_once(system.runtime, NULL, &delta) != 0 ||
            (delta.flags & STARSEAD_DELTA_FATAL) != 0U) {
            status = 1;
            system.shutdown_requested = true;
        }
    }
    system_runtime_finish_control_stop(&system);
    starsead_control_server_close_listener(system.control);
    if (!starsead_control_server_drained(system.control)) {
        int64_t now = 0;
        if (system_runtime_clock(&system, &now) == 0) {
            struct starsead_deadline deadline = {
                .armed = true,
                .monotonic_milliseconds = now + STARSEAD_CONTROL_WATCH_STALL_MILLIS,
            };
            struct starsead_runtime_delta ignored;
            (void)starsead_runtime_pump_until(system.runtime, &deadline,
...
int starsead_runtime_start_system(const char *config_path,
    bool *has_early_result, struct starsead_control_result *early_result) {
    return system_runtime_run(config_path, true, has_early_result, early_result);
}

int starsead_runtime_monitor_system(const char *config_path,
    bool *has_early_result, struct starsead_control_result *early_result) {
...
int starsead_runtime_start_system(
    ...
    *has_early_result = true;
    memset(early_result, 0, sizeof(*early_result));
    early_result->code = STARSEAD_CONTROL_RESULT_INTERNAL_ERROR;
...
void starsead_runtime_destroy(struct starsead_runtime *runtime) {
    free(runtime);
}
