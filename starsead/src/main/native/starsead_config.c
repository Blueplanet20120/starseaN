...
bool starsead_mode_is_readable(uint32_t mode) { return (mode & 0444U) != 0U; }
bool starsead_mode_is_writable(uint32_t mode) { return (mode & 0222U) != 0U; }
bool starsead_mode_is_executable(uint32_t mode) { return (mode & 0111U) != 0U; }

bool starsead_file_requirements_valid(
    enum starsead_file_kind actual,
    uint32_t mode,
    enum starsead_file_kind expected,
    bool readable,
    bool writable,
    bool executable) {
    return actual == expected &&
        (!readable || starsead_mode_is_readable(mode)) &&
        (!writable || starsead_mode_is_writable(mode)) &&
        (!executable || starsead_mode_is_executable(mode));
}
...
int starsead_file_requirements_validate(
    ...
    bool writable,
    bool executable) {
    return starsead_file_requirements_valid(
        actual, mode, expected, readable, writable, executable) ? 0 : STARSEAD_CONFIG_INVALID;
}

void starsead_config_destroy(struct starsead_config *config) {
...
static int copy_string(const struct starsead_json_document *, size_t, char *, size_t);
...
static int object_fields(
    ...
    size_t name_count,
    size_t *values) {
    if (document->tokens[object].type != STARSEAD_JSON_OBJECT || name_count > 64U) return -1;
    ...
    while (true) {
        ...
        if (value == TOKEN_NONE || document->tokens[key].type != STARSEAD_JSON_STRING) return -1;
...
static int copy_string(
    ...
    if (token->type != STARSEAD_JSON_STRING || out_size == 0U) return -1;
    size_t output = 0U;
...
static int parse_bool(const struct starsead_json_document *document, size_t token, bool *out) {
...
static int parse_u32(
    ...
    uint32_t *out) {
    const struct starsead_json_token *token = &document->tokens[token_index];
    if (token->type != STARSEAD_JSON_NUMBER || token->end <= token->start) return -1;
    uint64_t value = 0U;
...
static bool interface_is_valid(const char *value, bool prefix) {
    ...
    if (length == 0U || length >= STARSEAD_MAX_INTERFACE_NAME || (prefix && length == 1U && value[0] == '+')) return false;
...
static int parse_string_array(
    ...
    bool prefixes,
    size_t *count) {
    if (document->tokens[array].type != STARSEAD_JSON_ARRAY ||
...
static int normalize_cidr(
    ...
    char normalized[STARSEAD_MAX_CIDR];
    if (family == AF_INET) {
...
static int parse_direct_resource_family(
    ...
    size_t length,
    int family,
    char (**out)[STARSEAD_MAX_CIDR],
    size_t *count) {
    if (data == NULL && length != 0U) return STARSEAD_CONFIG_INVALID;
    *count = 0U;
    *out = calloc(STARSEAD_MAX_DIRECT_CIDRS, sizeof(**out));
    if (*out == NULL) return STARSEAD_CONFIG_NO_MEMORY;
    size_t cursor = 0U;
    while (cursor < length) {
        size_t end = cursor;
        ...
        if (content_end > cursor) {
            ...
            if (value_length >= STARSEAD_MAX_CIDR || *count >= STARSEAD_MAX_DIRECT_CIDRS) {
                return STARSEAD_CONFIG_INVALID;
            }
            char value[STARSEAD_MAX_CIDR];
            memcpy(value, data + cursor, value_length);
            value[value_length] = '\0';
            if (normalize_cidr(value, family, (*out)[*count], STARSEAD_MAX_CIDR, NULL) != 0) {
                return STARSEAD_CONFIG_INVALID;
            }
            ++*count;
        }
    ...
    for (size_t index = 0U; index < *count; ++index) {
        if (unique == 0U || strcmp((*out)[unique - 1U], (*out)[index]) != 0) {
            if (unique != index) memcpy((*out)[unique], (*out)[index], STARSEAD_MAX_CIDR);
            ++unique;
        }
    }
    *count = unique;
...
int starsead_config_load_direct_cidrs(
    ...
    if (config->direct_cidrs == NULL) return STARSEAD_CONFIG_NO_MEMORY;
...
static int parse_core(
    ...
    if (document->tokens[values[2]].type == STARSEAD_JSON_NULL) return 0;
...
static int parse_policy(
    ...
    config->app_policy_mode = (enum starsead_app_policy_mode)mode;
    ...
    if (result != 0) return result;
    bool ipv4_null = document->tokens[values[3]].type == STARSEAD_JSON_NULL;
    bool ipv6_null = document->tokens[values[4]].type == STARSEAD_JSON_NULL;
    ...
    return config->app_policy_mode == STARSEAD_APP_POLICY_GLOBAL && config->uid_count != 0U ? -1 : 0;
}
...
static int parse_network(
    ...
    if (document->tokens[values[4]].type != STARSEAD_JSON_NULL) {
    ...
    if (parse_string_array(document, values[5], (char *)config->ignored_interfaces, STARSEAD_MAX_INTERFACE_NAME, STARSEAD_MAX_INTERFACES, false, false, &config->ignored_interface_count) != 0) return -1;
    for (size_t index = 0U; index < config->ignored_interface_count; ++index) {
        if (!starsead_interface_selector_valid(config->ignored_interfaces[index])) return -1;
    }
    if (parse_string_array(document, values[6], (char *)config->virtual_interfaces, STARSEAD_MAX_INTERFACE_NAME, STARSEAD_MAX_INTERFACES, true, false, &config->virtual_interface_count) != 0 ||
        parse_string_array(document, values[7], (char *)config->hotspot_interface_prefixes, STARSEAD_MAX_INTERFACE_NAME, STARSEAD_MAX_INTERFACES, true, true, &config->hotspot_interface_prefix_count) != 0 ||
        parse_string_array(document, values[8], (char *)config->proxy_private_cidrs, STARSEAD_MAX_CIDR, STARSEAD_MAX_CIDRS, false, false, &config->proxy_private_cidr_count) != 0 ||
        parse_string_array(document, values[9], (char *)config->bypass_private_cidrs, STARSEAD_MAX_CIDR, STARSEAD_MAX_CIDRS, false, false, &config->bypass_private_cidr_count) != 0 ||
        config->proxy_private_cidr_count + config->bypass_private_cidr_count > STARSEAD_MAX_CIDRS ||
...
static int parse_wifi_ssids(
    const struct starsead_json_document *document,
    size_t array,
    struct starsead_wifi_rule_config *rule) {
    size_t cursor;

    if (document->tokens[array].type != STARSEAD_JSON_ARRAY ||
        document->tokens[array].child_count > STARSEAD_MAX_WIFI_IDENTIFIERS) {
        return -1;
    }
    rule->ssid_count = 0U;
    cursor = array + 1U;
    while (rule->ssid_count < document->tokens[array].child_count) {
        char decoded[STARSEAD_MAX_WIFI_SSID_BYTES + 1U];
        size_t token = next_direct(document, array, cursor);
        size_t length;
        size_t previous;
        if (token == TOKEN_NONE ||
        ...
        if (length == 0U || length > STARSEAD_MAX_WIFI_SSID_BYTES) return -1;
...
static int parse_schedule_control(
    ...
    char start[STARSEAD_MAX_CRON_EXPRESSION + 1U];
    char stop[STARSEAD_MAX_CRON_EXPRESSION + 1U];
    size_t values[3];
    if (object_fields(document, object, names, 3U, values) != 0 ||
    ...
    if (master_enabled && schedule->enabled &&
        (starsead_cron_parse(start, &schedule->start) != 0 ||
            starsead_cron_parse(stop, &schedule->stop) != 0)) {
        return -1;
    }
    return 0;
}
...
static int parse_mode_options(
    ...
    if (document->tokens[values[0]].type != STARSEAD_JSON_NULL) {
        uint32_t port = 0U;
    ...
    if (document->tokens[values[1]].type != STARSEAD_JSON_NULL) {
...
static int parse_matcher(
    ...
    size_t token,
    struct starsead_config *config) {
    if (document->tokens[token].type == STARSEAD_JSON_NULL) return 0;
...
static int parse_port(const struct starsead_json_document *document, size_t token, uint16_t *out) {
    uint32_t value = 0U;
...
static int parse_hev(
    ...
    size_t values[12];
    struct starsead_hev_helper_config *hev = &config->helper.value.hev;
    ...
    if (document->tokens[values[7]].type != STARSEAD_JSON_NULL) {
    ...
    config->helper.type = STARSEAD_HELPER_HEV_SOCKS5_TUNNEL;
    return 0;
}
...
static int parse_bpf(
    ...
    size_t values[18];
    struct starsead_bpf_helper_config *bpf = &config->helper.value.bpf;
...
static int parse_helper(
    ...
    struct starsead_config *config) {
    if (document->tokens[token].type == STARSEAD_JSON_NULL) return 0;
    if (document->tokens[token].type != STARSEAD_JSON_OBJECT) return -1;
...
static int validate_cross_fields(struct starsead_config *config) {
    if (config->mode == STARSEAD_MODE_EBPF && config->readiness_timeout_milliseconds < 1000U) return -1;
    if (config->core_type != STARSEAD_CORE_MIHOMO && config->has_age_secret_key) return -1;
    ...
    if ((config->mode == STARSEAD_MODE_TPROXY) != config->has_transparent_port ||
        (config->mode == STARSEAD_MODE_TUN) != config->has_tunnel_name) return -1;
    if (config->mode != STARSEAD_MODE_TPROXY && config->has_transparent_port) return -1;
    if (config->mode != STARSEAD_MODE_TUN && config->has_tunnel_name) return -1;
    if ((config->mode == STARSEAD_MODE_TUN2SOCKS && config->helper.type != STARSEAD_HELPER_HEV_SOCKS5_TUNNEL) ||
        (config->mode == STARSEAD_MODE_BPF2SOCKS && config->helper.type != STARSEAD_HELPER_BPF2SOCKS) ||
        ((config->mode == STARSEAD_MODE_TPROXY || config->mode == STARSEAD_MODE_TUN || config->mode == STARSEAD_MODE_EBPF) && config->helper.type != STARSEAD_HELPER_NONE)) return -1;
    if (config->matcher.enabled && !(config->mode == STARSEAD_MODE_TPROXY || config->mode == STARSEAD_MODE_TUN2SOCKS)) return -1;
    bool direct_consumer = config->matcher.enabled ^ (config->mode == STARSEAD_MODE_BPF2SOCKS);
    ...
    if (starsead_mode_core_managed(config->mode) &&
        (config->enable_local_dns || config->enable_fake_dns || config->has_fake_dns_ipv4_pool ||
         ...
         (config->mode == STARSEAD_MODE_EBPF && config->hotspot_interface_prefix_count != 0U) ||
         config->proxy_private_cidr_count != 0U ||
         config->bypass_private_cidr_count != 0U || config->app_policy_mode != STARSEAD_APP_POLICY_GLOBAL ||
         config->uid_count != 0U || config->bypass_uid_count != 0U || config->has_direct_cidr_paths ||
         config->matcher.enabled || config->helper.type != STARSEAD_HELPER_NONE ||
         config->has_transparent_port ||
         (config->mode == STARSEAD_MODE_EBPF && config->has_tunnel_name))) return -1;
    return 0;
}

static bool supported(const struct starsead_config *config) {
    if (config->owner == STARSEAD_OWNER_NG && config->core_type == STARSEAD_CORE_XRAY) {
        return config->mode == STARSEAD_MODE_TPROXY || config->mode == STARSEAD_MODE_TUN2SOCKS || config->mode == STARSEAD_MODE_BPF2SOCKS;
    }
    if (config->owner == STARSEAD_OWNER_BOX && config->core_type == STARSEAD_CORE_SING_BOX) return true;
    if (config->owner == STARSEAD_OWNER_META && config->core_type == STARSEAD_CORE_MIHOMO) return config->mode != STARSEAD_MODE_EBPF;
    return false;
}
...
int starsead_config_parse(
    ...
    char *message,
    size_t message_size) {
    if (config == NULL) return STARSEAD_CONFIG_INVALID;
    memset(config, 0, sizeof(*config));
    struct starsead_json_document document;
    int result = starsead_json_parse(json, length, &document, message, message_size);
    if (result != 0) return result;
    ...
    static const char *const owners[] = {"starsean", "asteriskbox", "asteriskmeta"};
    ...
    config->owner = (enum starsead_owner)owner;
    config->core_type = (enum starsead_core_type)core;
    config->mode = (enum starsead_mode)mode;
    ...
    if (!supported(config)) {
        ...
        starsead_config_destroy(config);
...
void starsead_runtime_directory_release(struct starsead_runtime_directory *directory) {
...
void starsead_loaded_config_release(struct starsead_loaded_config *loaded) {
    ...
    starsead_runtime_directory_release(&loaded->directory);
    starsead_config_destroy(&loaded->config);
...
static int load_direct_resources_with_backend(
    struct starsead_config *config,
    int directory_fd,
    const struct starsead_config_load_backend *backend,
    void *context) {
    if (!config->has_direct_cidr_paths) return 0;
    if (backend->read_resource == NULL) return STARSEAD_CONFIG_INVALID;
    char *ipv4 = NULL;
    ...
    if (result == 0 && (ipv4 == NULL && ipv4_length != 0U)) result = STARSEAD_CONFIG_INVALID;
    if (result == 0 && ipv4_length > STARSEAD_MAX_JSON_SIZE) result = STARSEAD_CONFIG_INVALID;
    if (result == 0 && config->enable_ipv6) {
        result = backend->read_resource(
            context, directory_fd, config->direct_cidr_path_v6, &ipv6, &ipv6_length);
        if (result == 0 && (ipv6 == NULL && ipv6_length != 0U)) result = STARSEAD_CONFIG_INVALID;
        if (result == 0 && ipv6_length > STARSEAD_MAX_JSON_SIZE) result = STARSEAD_CONFIG_INVALID;
    }
    if (result == 0) {
...
int starsead_config_load_with_backend(
    const char *path,
    struct starsead_loaded_config *loaded,
    const struct starsead_config_load_backend *backend,
    void *context,
    char *message,
    size_t message_size) {
    ...
    if (result == 0 && loaded->directory.fd < 0) result = STARSEAD_CONFIG_INVALID;
    if (result != 0) {
    ...
    if (result == 0 && loaded->config_fd < 0) result = STARSEAD_CONFIG_INVALID;
    if (result == 0 && json == NULL && length != 0U) result = STARSEAD_CONFIG_INVALID;
    if (result == 0 && length > STARSEAD_MAX_JSON_SIZE) result = STARSEAD_CONFIG_INVALID;
    if (result == 0) result = starsead_config_parse(json, length, &loaded->config, message, message_size);
    free(json);
...
#ifndef _WIN32
static enum starsead_file_kind file_kind(mode_t mode) {
...
static int checked_stat(
    ...
    struct stat *out) {
    struct stat status;
    if (fstat(fd, &status) != 0) return STARSEAD_CONFIG_IO;
    int result = starsead_file_requirements_validate(
...
static int open_parent_walk(const char *path, int *parent_fd, char *leaf) {
    if (!path_is_normal(path) || parent_fd == NULL || leaf == NULL) return STARSEAD_CONFIG_INVALID;
    ...
    while (result == 0) {
        ...
        if (next < 0) {
            result = path_open_error(errno, STARSEAD_CONFIG_INVALID);
            break;
        }
        result = checked_stat(next, STARSEAD_FILE_DIRECTORY, false, false, true, NULL);
        close(current);
        current = next;
        component = slash + 1;
    }
...
static int open_verified_absolute(
    ...
    int parent_fd = -1;
    char leaf[STARSEAD_MAX_PATH];
    int result = open_parent_walk(path, &parent_fd, leaf);
    if (result != 0) return result;
    if (writable_parent) {
        result = checked_stat(parent_fd, STARSEAD_FILE_DIRECTORY, false, true, true, NULL);
    }
    int probe_fd = -1;
    if (result == 0) {
    ...
    if (result == 0) {
        ...
        if (expected == STARSEAD_FILE_DIRECTORY) flags |= O_DIRECTORY;
    ...
    if (result == 0) {
        ...
        if (result == 0 && !same_inode(&probed, &accessed)) result = STARSEAD_CONFIG_INVALID;
    }
...
static int validate_loaded_paths(const struct starsead_config *config) {
    ...
    if (config->helper.type == STARSEAD_HELPER_HEV_SOCKS5_TUNNEL) helper = config->helper.value.hev.executable_path;
    if (config->helper.type == STARSEAD_HELPER_BPF2SOCKS) helper = config->helper.value.bpf.executable_path;
...
static int read_fd_bounded(int fd, char **out, size_t *length) {
    ...
    if (fstat(fd, &status) != 0 || !S_ISREG(status.st_mode) || status.st_size < 0 ||
        (uint64_t)status.st_size > STARSEAD_MAX_JSON_SIZE) return STARSEAD_CONFIG_INVALID;
    size_t size = (size_t)status.st_size;
    char *data = malloc(size == 0U ? 1U : size);
    if (data == NULL) return STARSEAD_CONFIG_NO_MEMORY;
    size_t offset = 0U;
    while (offset < size) {
...
int starsead_runtime_directory_open(
    const char *path,
    struct starsead_runtime_directory *directory_handle,
    char *message,
    size_t message_size) {
    if (directory_handle == NULL) return STARSEAD_CONFIG_INVALID;
    ...
    if (result != 0) {
        starsead_runtime_directory_release(directory_handle);
        return result;
    }
...
int starsead_config_load(
    const char *path,
    struct starsead_loaded_config *loaded,
    char *message,
    size_t message_size) {
    if (loaded == NULL) return STARSEAD_CONFIG_INVALID;
    memset(loaded, 0, sizeof(*loaded));
    loaded->directory.fd = -1;
    loaded->config_fd = -1;
    int result = starsead_runtime_directory_open(
...
int starsead_load_config(
    ...
    memset(config, 0, sizeof(*config));
    struct starsead_loaded_config loaded;
    int result = starsead_config_load(path, &loaded, message, message_size);
    if (result != 0) return result;
