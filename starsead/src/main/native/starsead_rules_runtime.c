...
int starsead_ip_rule_output_classify(
    const char *bytes, size_t length, const struct starsead_route_effect *effect,
    enum starsead_rules_slot_state *state) {
    if (effect == NULL || state == NULL || effect->kind != STARSEAD_ROUTE_EFFECT_IP_RULE ||
        (effect->family != STARSEAD_IP_FAMILY_IPV4 &&
         effect->family != STARSEAD_IP_FAMILY_IPV6) || effect->table == 0U ||
    ...
    *state = matches == 1U ? STARSEAD_RULES_SLOT_OWNED : STARSEAD_RULES_SLOT_ABSENT;
    return 0;
}
...
int starsead_ip_route_output_classify(
    const char *bytes, size_t length, const struct starsead_route_effect *effect,
    enum starsead_rules_slot_state *state) {
    if (effect == NULL || state == NULL || effect->kind != STARSEAD_ROUTE_EFFECT_ROUTE ||
        (effect->family != STARSEAD_IP_FAMILY_IPV4 &&
         effect->family != STARSEAD_IP_FAMILY_IPV6) || effect->table == 0U ||
...
size_t starsead_xtables_fake_dns_arguments(
    const char *pool, const char **arguments) {
    size_t pool_length = pool == NULL ? 0U : strnlen(pool, STARSEAD_MAX_CIDR);
    if (arguments == NULL || pool_length == 0U || pool_length >= STARSEAD_MAX_CIDR ||
        strpbrk(pool, " \t\r\n") != NULL) return 0U;
    arguments[0] = "-d";
    arguments[1] = pool;
    arguments[2] = "-p";
...
int starsead_xtables_private_chain_counts(
    ...
    size_t chain_length = strnlen(chain, STARSEAD_MAX_CHAIN_NAME);
    if (chain_length == 0U || chain_length >= STARSEAD_MAX_CHAIN_NAME ||
        bytes[length - 1U] != '\n') return -1;
...
size_t starsead_xtables_hook_arguments(
    ...
    arguments[count++] = "-j";
    if (hook->verdict == STARSEAD_HOOK_JUMP) {
        arguments[count++] = hook->jump_target;
    } else if (hook->verdict == STARSEAD_HOOK_DROP) {
        arguments[count++] = "DROP";
    } else if (hook->verdict == STARSEAD_HOOK_REJECT) {
...
int starsead_xtables_rule_output_locate(
    ...
    size_t chain_length = strnlen(chain, STARSEAD_MAX_CHAIN_NAME);
    if (chain_length == 0U || chain_length >= STARSEAD_MAX_CHAIN_NAME) return -1;

    char expected[STARSEAD_MAX_CHILD_ARG * 9U];
    ...
    for (size_t index = 0U; index < argument_count; ++index) {
        ...
        size_t argument_length = argument == NULL ? 0U :
            strnlen(argument, STARSEAD_MAX_CHILD_ARG);
        if (argument_length == 0U || argument_length >= STARSEAD_MAX_CHILD_ARG ||
            strpbrk(argument, " \t\r\n") != NULL ||
...
void starsead_rules_runtime_init(struct starsead_rules_runtime *runtime) {
    if (runtime == NULL) return;
    memset(runtime, 0, sizeof(*runtime));
    runtime->initialized = true;
}

static bool rules_backend_complete(const struct starsead_rules_backend *backend) {
...
static bool runtime_has_cleanup(const struct starsead_rules_runtime *runtime) {
    for (size_t index = 0U; index < STARSEAD_RULE_TRANSACTION_MAX_GROUPS; ++index) {
        if (runtime->private_cleanup_required[index] || runtime->hook_cleanup_required[index]) {
            return true;
        }
    }
    for (size_t index = 0U; index < STARSEAD_RULE_TRANSACTION_MAX_ROUTES; ++index) {
        if (runtime->route_cleanup_required[index]) return true;
    }
    return false;
}

int starsead_rules_install(
    struct starsead_rules_runtime *runtime,
    const struct starsead_config *config,
    bool has_global_ipv6_address,
    const struct starsead_rules_backend *backend) {
    if (runtime == NULL || !runtime->initialized || config == NULL || !rules_backend_complete(backend) ||
        runtime->installed || runtime_has_cleanup(runtime)) return STARSEAD_CONFIG_INVALID;
    struct starsead_rule_transaction_plan plan;
    int result = starsead_rule_transaction_plan_build(config, has_global_ipv6_address, &plan);
...
static int verify_private(
    const struct starsead_rules_runtime *runtime,
    const struct starsead_rules_backend *backend) {
    for (size_t index = 0U; index < runtime->plan.private_group_count; ++index) {
        ...
        if (backend->probe_private(backend->ctx, &runtime->plan.private_groups[index], &state) != 0 ||
            state != STARSEAD_RULES_SLOT_OWNED) return STARSEAD_CONFIG_IO;
    }
    return 0;
}
...
static int reconcile_private(
    ...
    for (size_t index = 0U; index < runtime->plan.private_group_count; ++index) {
        ...
        enum starsead_rules_slot_state state = STARSEAD_RULES_SLOT_FOREIGN;
        if (backend->probe_private(backend->ctx, &runtime->plan.private_groups[index], &state) != 0 ||
            state == STARSEAD_RULES_SLOT_FOREIGN) return STARSEAD_CONFIG_IO;
